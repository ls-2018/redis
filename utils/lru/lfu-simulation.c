...
void show_entry(long pos, struct entry *e) {
    ...
    printf("%ld] <%s> frequency:%d decrtime:%d [%lu hits | age:%ld sec]\n", pos, tag, e->counter, e->decrtime, (unsigned long)e->hits, time(NULL) - e->ctime);
}
...
int main(void) {
    ...
    while (1) {
        ...
        if (display_time != now) {
            ...
            for (j = 0; j < 20; j++) show_entry(j, entries + j);

            for (j = keyspace_size - 20; j < keyspace_size; j++) show_entry(j, entries + j);
            display_time = now;
        }
    }
    return 0;
