...
#define JEMALLOC_GENERATE_ATOMICS(type, short_type, /* unused */ lg_size)                                                                                                                                 \
    ...
    ATOMIC_INLINE type atomic_exchange_##short_type(atomic_##short_type##_t *a, type val, atomic_memory_order_t mo) {                                                                                     \
        ...
        return result;                                                                                                                                                                                    \
                                                                                                                                                                                                          ...
                                                                                                                                                                                                          \
    ATOMIC_INLINE bool atomic_compare_exchange_weak_##short_type(atomic_##short_type##_t *a, UNUSED type *expected, type desired, atomic_memory_order_t success_mo, atomic_memory_order_t failure_mo) {   \
        return __atomic_compare_exchange(&a->repr, expected, &desired, true, atomic_enum_to_builtin(success_mo), atomic_enum_to_builtin(failure_mo));                                                     \
    }                                                                                                                                                                                                     \
                                                                                                                                                                                                          \
    ATOMIC_INLINE bool atomic_compare_exchange_strong_##short_type(atomic_##short_type##_t *a, UNUSED type *expected, type desired, atomic_memory_order_t success_mo, atomic_memory_order_t failure_mo) { \
        return __atomic_compare_exchange(&a->repr, expected, &desired, false, atomic_enum_to_builtin(success_mo), atomic_enum_to_builtin(failure_mo));                                                    \
    }
