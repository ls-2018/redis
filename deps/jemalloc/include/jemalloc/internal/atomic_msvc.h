...
typedef enum { atomic_memory_order_relaxed, atomic_memory_order_acquire, atomic_memory_order_release, atomic_memory_order_acq_rel, atomic_memory_order_seq_cst } atomic_memory_order_t;
...
#define JEMALLOC_GENERATE_ATOMICS(type, short_type, lg_size)                                                                                                                                       \
    ...
    ATOMIC_INLINE type atomic_exchange_##short_type(atomic_##short_type##_t *a, type val, atomic_memory_order_t mo) {                                                                              \
        return (type)ATOMIC_INTERLOCKED_NAME(_InterlockedExchange, lg_size)(&a->repr, (ATOMIC_INTERLOCKED_REPR(lg_size))val);                                                                      \
    }                                                                                                                                                                                              \
                                                                                                                                                                                                   \
    ATOMIC_INLINE bool atomic_compare_exchange_weak_##short_type(atomic_##short_type##_t *a, type *expected, type desired, atomic_memory_order_t success_mo, atomic_memory_order_t failure_mo) {   \
        ATOMIC_INTERLOCKED_REPR(lg_size) e = (ATOMIC_INTERLOCKED_REPR(lg_size)) * expected;                                                                                                        \
        ATOMIC_INTERLOCKED_REPR(lg_size) d = (ATOMIC_INTERLOCKED_REPR(lg_size))desired;                                                                                                            \
        ATOMIC_INTERLOCKED_REPR(lg_size) old = ATOMIC_INTERLOCKED_NAME(_InterlockedCompareExchange, lg_size)(&a->repr, d, e);                                                                      \
        if (old == e) {                                                                                                                                                                            \
            return true;                                                                                                                                                                           \
        }                                                                                                                                                                                          \
        else {                                                                                                                                                                                     \
            *expected = (type)old;                                                                                                                                                                 \
            return false;                                                                                                                                                                          \
                                                                                                                                                                                                   ...
                                                                                                                                                                                                   \
    ATOMIC_INLINE bool atomic_compare_exchange_strong_##short_type(atomic_##short_type##_t *a, type *expected, type desired, atomic_memory_order_t success_mo, atomic_memory_order_t failure_mo) { \
...
#define JEMALLOC_GENERATE_INT_ATOMICS(type, short_type, lg_size)                                                                 \
    ...
    ATOMIC_INLINE type atomic_fetch_add_##short_type(atomic_##short_type##_t *a, type val, atomic_memory_order_t mo) {           \
        return (type)ATOMIC_INTERLOCKED_NAME(_InterlockedExchangeAdd, lg_size)(&a->repr, (ATOMIC_INTERLOCKED_REPR(lg_size))val); \
    }                                                                                                                            \
                                                                                                                                 \
    ...
    ATOMIC_INLINE type atomic_fetch_and_##short_type(atomic_##short_type##_t *a, type val, atomic_memory_order_t mo) {           \
        return (type)ATOMIC_INTERLOCKED_NAME(_InterlockedAnd, lg_size)(&a->repr, (ATOMIC_INTERLOCKED_REPR(lg_size))val);         \
    }                                                                                                                            \
    ATOMIC_INLINE type atomic_fetch_or_##short_type(atomic_##short_type##_t *a, type val, atomic_memory_order_t mo) {            \
        return (type)ATOMIC_INTERLOCKED_NAME(_InterlockedOr, lg_size)(&a->repr, (ATOMIC_INTERLOCKED_REPR(lg_size))val);          \
    }                                                                                                                            \
    ATOMIC_INLINE type atomic_fetch_xor_##short_type(atomic_##short_type##_t *a, type val, atomic_memory_order_t mo) {           \
        return (type)ATOMIC_INTERLOCKED_NAME(_InterlockedXor, lg_size)(&a->repr, (ATOMIC_INTERLOCKED_REPR(lg_size))val);         \
    }
