...
JEMALLOC_ALWAYS_INLINE pszind_t sz_psz2ind(size_t psz) {
    ...
    pszind_t shift = (x < SC_LG_NGROUP + LG_PAGE) ? 0 : x - (SC_LG_NGROUP + LG_PAGE);
    pszind_t grp = shift << SC_LG_NGROUP;

    pszind_t lg_delta = (x < SC_LG_NGROUP + LG_PAGE + 1) ? LG_PAGE : x - SC_LG_NGROUP - 1;

    size_t   delta_inverse_mask = ZU(-1) << lg_delta;
    pszind_t mod = ((((psz - 1) & delta_inverse_mask) >> lg_delta)) & ((ZU(1) << SC_LG_NGROUP) - 1);
...
static inline size_t sz_pind2sz_compute(pszind_t pind) {
    ...
    size_t grp_size = ((ZU(1) << (LG_PAGE + (SC_LG_NGROUP - 1))) << grp) & grp_size_mask;
...
static inline size_t sz_index2size_compute(szind_t index) {
...
#endif
    {
        ...
        size_t grp_size_mask = ~((!!grp) - 1);
        size_t grp_size = ((ZU(1) << (LG_QUANTUM + (SC_LG_NGROUP - 1))) << grp) & grp_size_mask;
