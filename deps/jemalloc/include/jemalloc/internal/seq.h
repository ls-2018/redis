...
#define seq_define(type, short_type)                                                     \
    typedef struct {                                                                     \
        atomic_zu_t seq;                                                                 \
        atomic_zu_t data[(sizeof(type) + sizeof(size_t) - 1) / sizeof(size_t)];          \
    } seq_##short_type##_t;                                                              \
                                                                                         \
    /*                                                                                   \
     ...
     */                                                                                  \
    static inline void seq_store_##short_type(seq_##short_type##_t *dst, type *src) {    \
    ...
    static inline bool seq_try_load_##short_type(type *dst, seq_##short_type##_t *src) { \
