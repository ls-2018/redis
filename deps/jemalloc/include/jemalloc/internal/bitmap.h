...
#define BITMAP_INFO_INITIALIZER(nbits)                                                                                                                                                                                                                                                     \
    {                                                                                                                                                                                                                                                                                      \
        /* nbits. */                                                                                                                                                                                                                                                                       \
        nbits,                                                                                                                                                                                                                         /* nlevels. */                                      \
            (BITMAP_GROUPS_L0(nbits) > BITMAP_GROUPS_L1(nbits)) + (BITMAP_GROUPS_L1(nbits) > BITMAP_GROUPS_L2(nbits)) + (BITMAP_GROUPS_L2(nbits) > BITMAP_GROUPS_L3(nbits)) + (BITMAP_GROUPS_L3(nbits) > BITMAP_GROUPS_L4(nbits)) + 1, /* levels. */                                       \
        {                                                                                                                                                                                                                                                                                  \
            {0}, {BITMAP_GROUPS_L0(nbits)}, {BITMAP_GROUPS_L1(nbits) + BITMAP_GROUPS_L0(nbits)}, {BITMAP_GROUPS_L2(nbits) + BITMAP_GROUPS_L1(nbits) + BITMAP_GROUPS_L0(nbits)}, {BITMAP_GROUPS_L3(nbits) + BITMAP_GROUPS_L2(nbits) + BITMAP_GROUPS_L1(nbits) + BITMAP_GROUPS_L0(nbits)}, { \
                BITMAP_GROUPS_L4(nbits) + BITMAP_GROUPS_L3(nbits) + BITMAP_GROUPS_L2(nbits) + BITMAP_GROUPS_L1(nbits) + BITMAP_GROUPS_L0(nbits)                                                                                                                                            \
            }                                                                                                                                                                                                                                                                              \
        }                                                                                                                                                                                                                                                                                  \
    }
...
static inline bool bitmap_get(bitmap_t *bitmap, const bitmap_info_t *binfo, size_t bit) {
    size_t   goff;
    bitmap_t g;
...
static inline void bitmap_set(bitmap_t *bitmap, const bitmap_info_t *binfo, size_t bit) {
    size_t    goff;
    bitmap_t *gp;
    bitmap_t  g;
...
static inline size_t bitmap_ffu(const bitmap_t *bitmap, const bitmap_info_t *binfo, size_t min_bit) {
    ...
    for (unsigned level = binfo->nlevels; level--;) {
        ...
        bitmap_t group = bitmap[binfo->levels[level].group_offset + (bit >> lg_bits_per_group)];
        unsigned group_nmask = (unsigned)(((min_bit > bit) ? (min_bit - bit) : 0) >> (lg_bits_per_group - LG_BITMAP_GROUP_NBITS));
        ...
        bit += ((size_t)(ffs_lu(group_masked) - 1)) << (lg_bits_per_group - LG_BITMAP_GROUP_NBITS);
    }
    assert(bit >= min_bit);
    assert(bit < binfo->nbits);
    return bit;
...
static inline void bitmap_unset(bitmap_t *bitmap, const bitmap_info_t *binfo, size_t bit) {
    size_t    goff;
    bitmap_t *gp;
    bitmap_t  g;
    UNUSED
