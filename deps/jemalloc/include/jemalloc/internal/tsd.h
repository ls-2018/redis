...
#define TSD_INITIALIZER \
    { ATOMIC_INIT(tsd_state_uninitialized), TCACHE_ENABLED_ZERO_INITIALIZER, false, 0, 0, 0, 0, 0, 0, NULL, RTREE_CTX_ZERO_INITIALIZER, NULL, NULL, NULL, TSD_BINSHARDS_ZERO_INITIALIZER, TCACHE_ZERO_INITIALIZER, WITNESS_TSD_INITIALIZER MALLOC_TEST_TSD_INITIALIZER }
...
#define O(n, t, nt)                                                                                                                                                                                 \
    JEMALLOC_ALWAYS_INLINE t *tsd_##n##p_get(tsd_t *tsd) {                                                                                                                                          \
        ...
        assert(state == tsd_state_nominal || state == tsd_state_nominal_slow || state == tsd_state_nominal_recompute || state == tsd_state_reincarnated || state == tsd_state_minimal_initialized); \
...
JEMALLOC_ALWAYS_INLINE void tsd_assert_fast(tsd_t *tsd) {
     ...
     */
    assert(!malloc_slow && tsd_tcache_enabled_get(tsd) && tsd_reentrancy_level_get(tsd) == 0);
}
