...
static void gen_mutex_ctl_str(char *str, size_t buf_len, const char *prefix, const char *mutex, const char *counter) {
    malloc_snprintf(str, buf_len, "stats.%s.%s.%s", prefix, mutex, counter);
}

static void mutex_stats_init_cols(emitter_row_t *row, const char *table_name, emitter_col_t *name, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters], emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters]) {
...
#undef WIDTH_uint64_t
        col_uint64_t[mutex_counter_total_wait_time_ps]
            .width = 10;
}

static void mutex_stats_read_global(const char *name, emitter_col_t *col_name, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters], emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters], uint64_t uptime) {
...
#define OP(counter, counter_type, human, derived, base_counter)                      \
    ...
    if (!derived) {                                                                  \
        gen_mutex_ctl_str(cmd, MUTEX_CTL_STR_MAX_LENGTH, "mutexes", name, #counter); \
...
static void mutex_stats_read_arena(unsigned arena_ind, mutex_prof_arena_ind_t mutex_ind, const char *name, emitter_col_t *col_name, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters], emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters], uint64_t uptime) {
    char cmd[MUTEX_CTL_STR_MAX_LENGTH];
...
#define OP(counter, counter_type, human, derived, base_counter)                                                       \
    ...
    dst->type = EMITTER_TYPE_##counter_type;                                                                          \
    if (!derived) {                                                                                                   \
        gen_mutex_ctl_str(cmd, MUTEX_CTL_STR_MAX_LENGTH, "arenas.0.mutexes", arena_mutex_names[mutex_ind], #counter); \
...
static void mutex_stats_read_arena_bin(unsigned arena_ind, unsigned bin_ind, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters], emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters], uint64_t uptime) {
    char           cmd[MUTEX_CTL_STR_MAX_LENGTH];
    emitter_col_t *dst;
...
#define OP(counter, counter_type, human, derived, base_counter)                                 \
    ...
    dst->type = EMITTER_TYPE_##counter_type;                                                    \
    if (!derived) {                                                                             \
        gen_mutex_ctl_str(cmd, MUTEX_CTL_STR_MAX_LENGTH, "arenas.0.bins.0", "mutex", #counter); \
        CTL_M2_M4_GET(cmd, arena_ind, bin_ind, (counter_type *)&dst->bool_val, counter_type);   \
    }                                                                                           \
    else {                                                                                      \
        emitter_col_t *base = &col_##counter_type[mutex_counter_##base_counter];                \
        dst->counter_type##_val = rate_per_second(base->counter_type##_val, uptime);            \
...
static void mutex_stats_emit(emitter_t *emitter, emitter_row_t *row, emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters], emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters]) {
    if (row != NULL) {
...
#define OP(counter, type, human, derived, base_counter)                                        \
    if (!derived) {                                                                            \
        ...
        ++k_##type;                                                                            \
        emitter_json_kv(emitter, #counter, EMITTER_TYPE_##type, (const void *)&col->bool_val); \
    }
    MUTEX_PROF_COUNTERS;
#undef OP
#undef EMITTER_TYPE_uint32_t
...
static void stats_arena_bins_print(emitter_t *emitter, bool mutex, unsigned i, uint64_t uptime) {
    size_t   page;
    ...
    if (mutex) {
        ...
        mutex_stats_init_cols(&header_row, NULL, NULL, header_mutex64, header_mutex32);
    }
    ...
    for (j = 0, in_gap = false; j < nbins; j++) {
        ...
        CTL_M2_M4_GET("stats.arenas.0.bins.0.nmalloc", i, j, &nmalloc, uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.bins.0.ndalloc", i, j, &ndalloc, uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.bins.0.curregs", i, j, &curregs, size_t);
        CTL_M2_M4_GET("stats.arenas.0.bins.0.nrequests", i, j, &nrequests, uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.bins.0.nfills", i, j, &nfills, uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.bins.0.nflushes", i, j, &nflushes, uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.bins.0.nreslabs", i, j, &nreslabs, uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.bins.0.curslabs", i, j, &curslabs, size_t);
        CTL_M2_M4_GET("stats.arenas.0.bins.0.nonfull_slabs", i, j, &nonfull_slabs, size_t);
        ...
        emitter_json_kv(emitter, "nonfull_slabs", emitter_type_size, &nonfull_slabs);
        if (mutex) {
...
static void stats_arena_lextents_print(emitter_t *emitter, unsigned i, uint64_t uptime) {
    ...
    for (j = 0, in_gap = false; j < nlextents; j++) {
        ...
        CTL_M2_M4_GET("stats.arenas.0.lextents.0.nmalloc", i, j, &nmalloc, uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.lextents.0.ndalloc", i, j, &ndalloc, uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.lextents.0.nrequests", i, j, &nrequests, uint64_t);
        in_gap_prev = in_gap;
        in_gap = (nrequests == 0);
        ...
        CTL_M2_M4_GET("stats.arenas.0.lextents.0.curlextents", i, j, &curlextents, size_t);
...
static void stats_arena_extents_print(emitter_t *emitter, unsigned i) {
    ...
    for (j = 0; j < SC_NPSIZES; j++) {
        size_t ndirty, nmuzzy, nretained, total, dirty_bytes, muzzy_bytes, retained_bytes, total_bytes;
        ...
        CTL_M2_M4_GET("stats.arenas.0.extents.0.nretained", i, j, &nretained, size_t);
        CTL_M2_M4_GET("stats.arenas.0.extents.0.dirty_bytes", i, j, &dirty_bytes, size_t);
        CTL_M2_M4_GET("stats.arenas.0.extents.0.muzzy_bytes", i, j, &muzzy_bytes, size_t);
        CTL_M2_M4_GET("stats.arenas.0.extents.0.retained_bytes", i, j, &retained_bytes, size_t);
        ...
        emitter_json_kv(emitter, "dirty_bytes", emitter_type_size, &dirty_bytes);
        emitter_json_kv(emitter, "muzzy_bytes", emitter_type_size, &muzzy_bytes);
        emitter_json_kv(emitter, "retained_bytes", emitter_type_size, &retained_bytes);
        emitter_json_object_end(emitter);
...
static void stats_arena_mutexes_print(emitter_t *emitter, unsigned arena_ind, uint64_t uptime) {
    emitter_row_t row;
    ...
    for (mutex_prof_arena_ind_t i = 0; i < mutex_prof_num_arena_mutexes; i++) {
        ...
        mutex_stats_read_arena(arena_ind, i, name, &col_name, col64, col32, uptime);
...
static void stats_arena_print(emitter_t *emitter, unsigned i, bool bins, bool large, bool mutex, bool extents) {
    unsigned    nthreads;
    const char *dss;
    ...
    uint64_t    small_nmalloc, small_ndalloc, small_nrequests, small_nfills, small_nflushes;
    size_t      large_allocated;
    uint64_t    large_nmalloc, large_ndalloc, large_nrequests, large_nfills, large_nflushes;
...
#define GET_AND_EMIT_ALLOC_STAT(small_or_large, name, valtype)                                         \
    CTL_M2_GET("stats.arenas.0." #small_or_large "." #name, i, &small_or_large##_##name, valtype##_t); \
    emitter_json_kv(emitter, #name, emitter_type_##valtype, &small_or_large##_##name);                 \
    ...
    col_count_nmalloc_ps.uint64_val = rate_per_second(col_count_nmalloc.uint64_val, uptime);
    GET_AND_EMIT_ALLOC_STAT(small, ndalloc, uint64)
    col_count_ndalloc_ps.uint64_val = rate_per_second(col_count_ndalloc.uint64_val, uptime);
    GET_AND_EMIT_ALLOC_STAT(small, nrequests, uint64)
    col_count_nrequests_ps.uint64_val = rate_per_second(col_count_nrequests.uint64_val, uptime);
    GET_AND_EMIT_ALLOC_STAT(small, nfills, uint64)
    col_count_nfills_ps.uint64_val = rate_per_second(col_count_nfills.uint64_val, uptime);
    GET_AND_EMIT_ALLOC_STAT(small, nflushes, uint64)
    col_count_nflushes_ps.uint64_val = rate_per_second(col_count_nflushes.uint64_val, uptime);
...
#define OPT_WRITE_MUTABLE(name, var1, var2, size, emitter_type, altname)                                                          \
    if (je_mallctl("opt." name, (void *)&var1, &size, NULL, 0) == 0 && je_mallctl(altname, (void *)&var2, &size, NULL, 0) == 0) { \
        emitter_kv_note(emitter, name, "opt." name, emitter_type, &var1, altname, emitter_type, &var2);                           \
    }
...
#undef OPT_WRITE_CHAR_P
    ...
    if (config_prof) {
        ...
        emitter_kv(emitter, "thread_active_init", "prof.thread_active_init", emitter_type_bool, &bv);
        ...
        emitter_kv(emitter, "interval", "prof.interval", emitter_type_uint64, &u64v);

        CTL_GET("prof.lg_sample", &ssv, ssize_t);
        emitter_kv(emitter, "lg_sample", "prof.lg_sample", emitter_type_ssize, &ssv);
    ...
    unsigned nbins;
    CTL_GET("arenas.nbins", &nbins, unsigned);
    emitter_kv(emitter, "nbins", "Number of bin size classes", emitter_type_unsigned, &nbins);

    unsigned nhbins;
    CTL_GET("arenas.nhbins", &nhbins, unsigned);
    emitter_kv(emitter, "nhbins", "Number of thread-cache bin size classes", emitter_type_unsigned, &nhbins);
...
static void stats_print_helper(emitter_t *emitter, bool merged, bool destroyed, bool unmerged, bool bins, bool large, bool mutex, bool extents) {
    /*
     * These should be deleted.  We keep them around for a while, to aid in
     * the transition to the emitter code.
     */
    size_t   allocated, active, metadata, metadata_thp, resident, mapped, retained;
    ...
    if (have_background_thread) {
        CTL_GET("stats.background_thread.num_threads", &num_background_threads, size_t);
        CTL_GET("stats.background_thread.num_runs", &background_thread_num_runs, uint64_t);
        CTL_GET("stats.background_thread.run_interval", &background_thread_run_interval, uint64_t);
    }
    else {
    ...
    emitter_json_kv(emitter, "num_threads", emitter_type_size, &num_background_threads);
    emitter_json_kv(emitter, "num_runs", emitter_type_uint64, &background_thread_num_runs);
    ...
    emitter_table_printf(
        ...
        num_background_threads, background_thread_num_runs, background_thread_run_interval);
    ...
    if (mutex) {
        ...
        for (int i = 0; i < mutex_prof_num_global_mutexes; i++) {
            mutex_stats_read_global(global_mutex_names[i], &name, col64, col32, uptime);
    ...
    if (merged || destroyed || unmerged) {
        ...
        if (merged && (ninitialized > 1 || !unmerged)) {
            ...
            stats_arena_print(emitter, MALLCTL_ARENAS_ALL, bins, large, mutex, extents);
        ...
        if (destroyed_initialized && destroyed) {
            ...
            stats_arena_print(emitter, MALLCTL_ARENAS_DESTROYED, bins, large, mutex, extents);
...
void stats_print(void (*write_cb)(void *, const char *), void *cbopaque, const char *opts) {
    int      err;
    uint64_t epoch;
    size_t   u64sz;
...
#undef OPTION
    ...
    err = je_mallctl("epoch", (void *)&epoch, &u64sz, (void *)&epoch, sizeof(uint64_t));
    if (err != 0) {
        if (err == EAGAIN) {
            malloc_write(
...
#undef OPTION
    ...
    emitter_t emitter;
    emitter_init(&emitter, json ? emitter_output_json : emitter_output_table, write_cb, cbopaque);
    ...
    if (config_stats) {
        stats_print_helper(&emitter, merged, destroyed, unmerged, bins, large, mutex, extents);
    }
