...
bool hook_boot() {
    return malloc_mutex_init(&hooks_mu, "hooks", WITNESS_RANK_HOOK, malloc_mutex_rank_exclusive);
}
...
static void *hook_install_locked(hooks_t *to_install) {
    ...
    for (int i = 0; i < HOOK_MAX; i++) {
        ...
        if (!hooks_internal.in_use) {
            ...
            atomic_store_u(&nhooks, atomic_load_u(&nhooks, ATOMIC_RELAXED) + 1, ATOMIC_RELAXED);
            return &hooks[i];
        }
    }
    return NULL;
...
void hook_remove(tsdn_t *tsdn, void *opaque) {
    if (config_debug) {
        ...
        assert(hooks_begin <= hook && hook < hooks_end && (hook - hooks_begin) % sizeof(seq_hooks_t) == 0);
    }
    malloc_mutex_lock(tsdn, &hooks_mu);
...
#define FOR_EACH_HOOK_BEGIN(hooks_internal_ptr)                                                               \
    for (int for_each_hook_counter = 0; for_each_hook_counter < HOOK_MAX; for_each_hook_counter++) {          \
        bool for_each_hook_success = seq_try_load_hooks((hooks_internal_ptr), &hooks[for_each_hook_counter]); \
...
void hook_invoke_alloc(hook_alloc_t type, void *result, uintptr_t result_raw, uintptr_t args_raw[3]) {
    HOOK_PROLOGUE
...
void hook_invoke_dalloc(hook_dalloc_t type, void *address, uintptr_t args_raw[3]) {
    HOOK_PROLOGUE
    hooks_internal_t hook;
    FOR_EACH_HOOK_BEGIN(&hook)
...
void hook_invoke_expand(hook_expand_t type, void *address, size_t old_usize, size_t new_usize, uintptr_t result_raw, uintptr_t args_raw[4]) {
    HOOK_PROLOGUE
    hooks_internal_t hook;
    FOR_EACH_HOOK_BEGIN(&hook)
    hook_expand h = hook.hooks.expand_hook;
    if (h != NULL) {
        h(hook.hooks.extra, type, address, old_usize, new_usize, result_raw, args_raw);
    }
    FOR_EACH_HOOK_END
    HOOK_EPILOGUE
}
