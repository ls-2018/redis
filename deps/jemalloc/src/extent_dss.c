...
static void extent_dss_extending_start(void) {
    ...
    while (true) {
        bool expected = false;
        if (atomic_compare_exchange_weak_b(&dss_extending, &expected, true, ATOMIC_ACQ_REL, ATOMIC_RELAXED)) {
            break;
        }
...
void *extent_alloc_dss(tsdn_t *tsdn, arena_t *arena, void *new_addr, size_t size, size_t alignment, bool *zero, bool *commit) {
    extent_t *gap;
    ...
    if (!atomic_load_b(&dss_exhausted, ATOMIC_ACQUIRE)) {
        ...
        while (true) {
            ...
            if (gap_size_page != 0) {
                extent_init(gap, arena, gap_addr_page, gap_size_page, false, SC_NSIZES, arena_extent_sn_next(arena), extent_state_active, false, true, true, EXTENT_NOT_HEAD);
            }
            /*
            ...
            if ((uintptr_t)ret < (uintptr_t)max_cur || (uintptr_t)dss_next < (uintptr_t)max_cur) {
                goto label_oom; /* Wrap-around. */
            }
            /* Compute the increment, including subpage bytes. */
            void    *gap_addr_subpage = max_cur;
            size_t   gap_size_subpage = (uintptr_t)ret - (uintptr_t)gap_addr_subpage;
            ...
            if (dss_prev == max_cur) {
                ...
                if (*zero && *commit) {
                    ...
                    extent_init(&extent, arena, ret, size, size, false, SC_NSIZES, extent_state_active, false, true, true, EXTENT_NOT_HEAD);
                    if (extent_purge_forced_wrapper(tsdn, arena, &extent_hooks, &extent, 0, size)) {
                        memset(ret, 0, size);
                    }
                }
                return ret;
...
static bool extent_in_dss_helper(void *addr, void *max) {
    return ((uintptr_t)addr >= (uintptr_t)dss_base && (uintptr_t)addr < (uintptr_t)max);
}
...
bool extent_dss_mergeable(void *addr_a, void *addr_b) {
    ...
    if ((uintptr_t)addr_a < (uintptr_t)dss_base && (uintptr_t)addr_b < (uintptr_t)dss_base) {
        return true;
    }
