...
void tcache_event_hard(tsd_t *tsd, tcache_t *tcache) {
    ...
    if (tbin->low_water > 0) {
         ...
         */
        if (binind < SC_NBINS) {
            tcache_bin_flush_small(tsd, tcache, tbin, binind, tbin->ncached - tbin->low_water + (tbin->low_water >> 2));
            /*
             * Reduce fill count by 2X.  Limit lg_fill_div such that
             * the fill count is always at least 1.
             */
            cache_bin_info_t *tbin_info = &tcache_bin_info[binind];
            if ((tbin_info->ncached_max >> (tcache->lg_fill_div[binind] + 1)) >= 1) {
                tcache->lg_fill_div[binind]++;
            }
        }
        else {
            tcache_bin_flush_large(tsd, tbin, binind, tbin->ncached - tbin->low_water + (tbin->low_water >> 2), tcache);
        }
    }
...
void *tcache_alloc_small_hard(tsdn_t *tsdn, arena_t *arena, tcache_t *tcache, cache_bin_t *tbin, szind_t binind, bool *tcache_success) {
    void *ret;

    assert(tcache->arena != NULL);
    arena_tcache_fill_small(tsdn, arena, tcache, tbin, binind, config_prof ? tcache->prof_accumbytes : 0);
    if (config_prof) {
...
static void tbin_extents_lookup_size_check(tsdn_t *tsdn, cache_bin_t *tbin, szind_t binind, size_t nflush, extent_t **extents) {
    ...
    for (unsigned i = 0; i < nflush; i++) {
        rtree_extent_szind_read(tsdn, &extents_rtree, rtree_ctx, (uintptr_t) * (tbin->avail - 1 - i), true, &extents[i], &szind);
        sz_sum -= szind;
    }
    if (sz_sum != 0) {
        safety_check_fail(
...
void tcache_bin_flush_small(tsd_t *tsd, tcache_t *tcache, cache_bin_t *tbin, szind_t binind, unsigned rem) {
    ...
    while (nflush > 0) {
        ...
        for (unsigned i = 0; i < nflush; i++) {
            ...
            if (extent_arena_ind_get(extent) == bin_arena_ind && extent_binshard_get(extent) == binshard) {
    ...
    if (config_stats && !merged_stats) {
         ...
         */
        unsigned binshard;
        bin_t   *bin = arena_bin_choose_lock(tsd_tsdn(tsd), arena, binind, &binshard);
        bin->stats.nflushes++;
        bin->stats.nrequests += tbin->tstats.nrequests;
        tbin->tstats.nrequests = 0;
        malloc_mutex_unlock(tsd_tsdn(tsd), &bin->lock);
...
void tcache_bin_flush_large(tsd_t *tsd, cache_bin_t *tbin, szind_t binind, unsigned rem, tcache_t *tcache) {
    bool merged_stats = false;
    ...
    while (nflush > 0) {
        ...
        arena_t  *locked_arena = arena_get(tsd_tsdn(tsd), locked_arena_ind, false);
        bool      idump;
        ...
        if ((config_prof || config_stats) && (locked_arena == tcache_arena)) {
            ...
            if (config_stats) {
                merged_stats = true;
                arena_stats_large_flush_nrequests_add(tsd_tsdn(tsd), &tcache_arena->stats, binind, tbin->tstats.nrequests);
                tbin->tstats.nrequests = 0;
            }
        }
        if (lock_large) {
...
static void tcache_arena_dissociate(tsdn_t *tsdn, tcache_t *tcache) {
    ...
    if (config_stats) {
        ...
        ql_remove(&arena->cache_bin_array_descriptor_ql, &tcache->cache_bin_array_descriptor, link);
...
static void tcache_init(tsd_t *tsd, tcache_t *tcache, void *avail_stack) {
    ...
    for (; i < SC_NBINS; i++) {
         ...
         * prefetch).
         */
        tcache_small_bin_get(tcache, i)->avail = (void **)((uintptr_t)avail_stack + (uintptr_t)stack_offset);
    }
    for (; i < nhbins; i++) {
        stack_offset += tcache_bin_info[i].ncached_max * sizeof(void *);
        tcache_large_bin_get(tcache, i)->avail = (void **)((uintptr_t)avail_stack + (uintptr_t)stack_offset);
    }
...
bool tsd_tcache_data_init(tsd_t *tsd) {
    ...
    void *avail_array = ipallocztm(tsd_tsdn(tsd), size, CACHELINE, true, NULL, true, arena_get(TSDN_NULL, 0, true));
    if (avail_array == NULL) {
        return true;
    }
...
static void tcache_flush_cache(tsd_t *tsd, tcache_t *tcache) {
    ...
    if (config_prof && tcache->prof_accumbytes > 0 && arena_prof_accum(tsd_tsdn(tsd), tcache->arena, tcache->prof_accumbytes)) {
        prof_idump(tsd_tsdn(tsd));
    }
}
...
static void tcache_destroy(tsd_t *tsd, tcache_t *tcache, bool tsd_tcache) {
    ...
    if (tsd_tcache) {
        ...
        void *avail_array = (void *)((uintptr_t)tcache_small_bin_get(tcache, 0)->avail - (uintptr_t)tcache_bin_info[0].ncached_max * sizeof(void *));
     ...
     */
    arena_decay(tsd_tsdn(tsd), arena_get(tsd_tsdn(tsd), 0, false), false, false);
...
void tcache_stats_merge(tsdn_t *tsdn, tcache_t *tcache, arena_t *arena) {
    ...
    for (; i < nhbins; i++) {
        cache_bin_t *tbin = tcache_large_bin_get(tcache, i);
        arena_stats_large_flush_nrequests_add(tsdn, &arena->stats, i, tbin->tstats.nrequests);
        tbin->tstats.nrequests = 0;
    }
}
...
static bool tcaches_create_prep(tsd_t *tsd) {
    ...
    if (tcaches == NULL) {
        tcaches = base_alloc(tsd_tsdn(tsd), b0get(), sizeof(tcache_t *) * (MALLOCX_TCACHE_MAX + 1), CACHELINE);
        if (tcaches == NULL) {
            err = true;
            goto label_return;
        }
...
static tcache_t *tcaches_elm_remove(tsd_t *tsd, tcaches_t *elm, bool allow_reinit) {
