...
static void *os_pages_map(void *addr, size_t size, size_t alignment, bool *commit) {
     ...
     */
    ret = VirtualAlloc(addr, size, MEM_RESERVE | (*commit ? MEM_COMMIT : 0), PAGE_READWRITE);
#else
    /*
...
static void *pages_map_slow(size_t size, size_t alignment, bool *commit) {
    ...
    do {
        ...
        size_t leadsize = ALIGNMENT_CEILING((uintptr_t)pages, alignment) - (uintptr_t)pages;
...
#ifdef _WIN32
    return (commit ? (addr != VirtualAlloc(addr, size, MEM_COMMIT, PAGE_READWRITE)) : (!VirtualFree(addr, size, MEM_DECOMMIT)));
#else
    {
        int   prot = commit ? PAGES_PROT_COMMIT : PAGES_PROT_DECOMMIT;
...
static bool os_overcommits_proc(void) {
    ...
    fd = (int)syscall(SYS_open, "/proc/sys/vm/overcommit_memory", O_RDONLY | O_CLOEXEC);
#else
...
#if defined(O_CLOEXEC)
    fd = (int)syscall(SYS_openat, AT_FDCWD, "/proc/sys/vm/overcommit_memory", O_RDONLY | O_CLOEXEC);
#else
    fd = (int)syscall(SYS_openat, AT_FDCWD, "/proc/sys/vm/overcommit_memory", O_RDONLY);
    if (fd != -1) {
        fcntl(fd, F_SETFD, fcntl(fd, F_GETFD) | FD_CLOEXEC);
    }
#endif
...
void pages_set_thp_state(void *ptr, size_t size) {
    ...
    if (opt_thp == thp_mode_always && init_system_thp_mode != thp_mode_never) {
        ...
        assert(init_system_thp_mode == thp_mode_default || init_system_thp_mode == thp_mode_always);
...
#if defined(JEMALLOC_USE_SYSCALL) && defined(SYS_open)
    int fd = (int)syscall(SYS_open, "/sys/kernel/mm/transparent_hugepage/enabled", O_RDONLY);
#else
