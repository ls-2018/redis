...
void *thd_start(void *arg) {
    ...
    assert_ptr_eq(ap0, ap1, "Pointer returned by \"thread.allocatedp\" should not change");
    ...
    assert_ptr_eq(dp0, dp1, "Pointer returned by \"thread.deallocatedp\" should not change");
    ...
    return NULL;
label_ENOENT:
    assert_false(config_stats, "ENOENT should only be returned if stats are disabled");
...
int main(void) {
    ...
    return test(test_main_thread, test_subthread, test_main_thread, test_subthread, test_main_thread);
}
