 ...
 * buf_descriptor_t *).
 */
static void forwarding_cb(void *buf_descriptor_v, const char *str) {
    ...
    size_t written = malloc_snprintf(buf_descriptor->buf, buf_descriptor->len, "%s", str);
...
static void assert_emit_output(void (*emit_fn)(emitter_t *), const char *expected_json_output, const char *expected_table_output) {
    emitter_t        emitter;
    ...
    emitter_init(&emitter, emitter_output_json, &forwarding_cb, &buf_descriptor);
    (*emit_fn)(&emitter);
    ...
    emitter_init(&emitter, emitter_output_table, &forwarding_cb, &buf_descriptor);
    (*emit_fn)(&emitter);
    assert_str_eq(expected_table_output, buf, "table output failure");
}

static void emit_dict(emitter_t *emitter) {
    bool        b_false = false;
    bool        b_true = true;
    int         i_123 = 123;
    ...
    emitter_kv_note(emitter, "ghi", "GHI", emitter_type_int, &i_123, "note_key1", emitter_type_string, &str);
    emitter_kv_note(emitter, "jkl", "JKL", emitter_type_string, &str, "note_key2", emitter_type_bool, &b_false);
...
static void emit_table_printf(emitter_t *emitter) {
    emitter_begin(emitter);
...
TEST_BEGIN(test_table_printf) {
    assert_emit_output(&emit_table_printf, table_printf_json, table_printf_table);
}
...
static void emit_nested_dict(emitter_t *emitter) {
    ...
    emitter_kv(emitter, "primitive", "Another primitive", emitter_type_int, &val);
...
TEST_BEGIN(test_nested_dict) {
    assert_emit_output(&emit_nested_dict, nested_object_json, nested_object_table);
}

TEST_END

static void emit_types(emitter_t *emitter) {
    bool        b = false;
    int         i = -123;
    unsigned    u = 123;
    ssize_t     zd = -456;
...
static void emit_json_arr(emitter_t *emitter) {
    int ival = 123;
...
static void emit_json_nested_array(emitter_t *emitter) {
    int   ival = 123;
    char *sval = "foo";
    emitter_begin(emitter);
    emitter_json_array_begin(emitter);
...
TEST_BEGIN(test_json_nested_arr) {
    assert_emit_output(&emit_json_nested_array, json_nested_array_json, json_array_table);
}

TEST_END

static void emit_table_row(emitter_t *emitter) {
    emitter_begin(emitter);
    emitter_row_t row;
...
int main(void) {
    return test_no_reentrancy(test_dict, test_table_printf, test_nested_dict, test_types, test_modal, test_json_arr, test_json_nested_arr, test_table_row);
}
