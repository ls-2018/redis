...
TEST_BEGIN(test_update_slab_size) {
    ...
    sc_data_init(&data);
    sc_t  *tiny = &data.sc[0];
    size_t tiny_size = (ZU(1) << tiny->lg_base) + (ZU(tiny->ndelta) << tiny->lg_delta);
    ...
    for (int i = 0; i < data.nbins; i++) {
        sc_t  *sc = &data.sc[i];
        size_t reg_size = (ZU(1) << sc->lg_base) + (ZU(sc->ndelta) << sc->lg_delta);
        if (reg_size <= PAGE) {
            assert_d_eq(sc->pgs, 1, "Ignored valid page size hint");
        }
        else {
            assert_d_gt(sc->pgs, 1, "Allowed invalid page size hint");
        }
    }
}
