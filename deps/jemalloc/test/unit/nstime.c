...
TEST_BEGIN(test_nstime_compare) {
    ...
    assert_d_eq(nstime_compare(&nsta, &nstb), 1, "nsta should be greater than nstb");
    assert_d_eq(nstime_compare(&nstb, &nsta), -1, "nstb should be less than nsta");

    nstime_init2(&nstb, 42, 44);
    assert_d_eq(nstime_compare(&nsta, &nstb), -1, "nsta should be less than nstb");
    assert_d_eq(nstime_compare(&nstb, &nsta), 1, "nstb should be greater than nsta");
...
TEST_BEGIN(test_nstime_add) {
    ...
    assert_d_eq(nstime_compare(&nsta, &nstb), 0, "Incorrect addition result");
...
TEST_BEGIN(test_nstime_subtract) {
    ...
    nstime_init(&nstb, 0);
    assert_d_eq(nstime_compare(&nsta, &nstb), 0, "Incorrect subtraction result");
...
TEST_BEGIN(test_nstime_imultiply) {
    ...
    assert_d_eq(nstime_compare(&nsta, &nstb), 0, "Incorrect multiplication result");
...
TEST_BEGIN(test_nstime_idivide) {
    ...
    assert_d_eq(nstime_compare(&nsta, &nstb), 0, "Incorrect division result");
...
TEST_BEGIN(test_nstime_divide) {
    ...
    nstime_copy(&nstb, &nsta);
    nstime_imultiply(&nsta, 10);
    assert_u64_eq(nstime_divide(&nsta, &nstb), 10, "Incorrect division result");
    ...
    assert_u64_eq(nstime_divide(&nsta, &nstb), 9, "Incorrect division result");
}
...
TEST_BEGIN(test_nstime_update) {
    ...
    }
    {
        ...
        assert_true(nstime_update(&nst), "Update should detect time roll-back.");
        assert_d_eq(nstime_compare(&nst, &nst0), 0, "Time should not have been modified");
    }
}
...
int main(void) {
    return test(test_nstime_init, test_nstime_init2, test_nstime_copy, test_nstime_compare, test_nstime_add, test_nstime_iadd, test_nstime_subtract, test_nstime_isubtract, test_nstime_imultiply, test_nstime_idivide, test_nstime_divide, test_nstime_monotonic, test_nstime_update);
}
