...
TEST_BEGIN(test_smoothstep_integral) {
    ...
    assert_u64_ge(sum, min, "Integral too small, even accounting for truncation");
...
TEST_BEGIN(test_smoothstep_monotonic) {
    ...
    assert_u64_eq(smoothstep_tab[SMOOTHSTEP_NSTEPS - 1], (KQU(1) << SMOOTHSTEP_BFP), "Last step must equal 1");
}
...
int main(void) {
    return test(test_smoothstep_integral, test_smoothstep_monotonic, test_smoothstep_slope);
}
