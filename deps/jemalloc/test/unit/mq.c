...
mq_gen(
    ...
    assert_ptr_null(mq_tryget(&mq), "mq_tryget() should fail when the queue is empty");
...
static void *thd_receiver_start(void *arg) {
    mq_t    *mq = (mq_t *)arg;
    unsigned i;
...
static void *thd_sender_start(void *arg) {
    mq_t    *mq = (mq_t *)arg;
    unsigned i;
