...
static void *thd_producer(void *varg) {
    void   **mem = varg;
    unsigned arena, i;
    size_t   sz;
...
TEST_BEGIN(test_producer_consumer) {
    ...
    for (i = 0; i < NTHREADS; i++) {
        for (unsigned j = 0; j < REMOTE_NALLOC; j++) {
            assert_ptr_not_null(mem[i][j], "Unexpected remote allocation failure");
            dallocx(mem[i][j], 0);
        }
    }
}
...
TEST_BEGIN(test_bin_shard) {
    ...
    miblen = 4;
    assert_d_eq(mallctlnametomib("arenas.bin.0.nshards", mib, &miblen), 0, "Unexpected mallctlnametomib() failure");
    miblen2 = 4;
    assert_d_eq(mallctlnametomib("arenas.bin.0.size", mib2, &miblen2), 0, "Unexpected mallctlnametomib() failure");
    ...
    for (i = 0; i < nbins; i++) {
        ...
        mib[2] = i;
        sz1 = sizeof(nshards);
        assert_d_eq(mallctlbymib(mib, miblen, (void *)&nshards, &sz1, NULL, 0), 0, "Unexpected mallctlbymib() failure");

        mib2[2] = i;
        sz2 = sizeof(size);
        assert_d_eq(mallctlbymib(mib2, miblen2, (void *)&size, &sz2, NULL, 0), 0, "Unexpected mallctlbymib() failure");
...
int main(void) {
    return test_no_reentrancy(test_bin_shard, test_bin_shard_mt, test_producer_consumer);
}
