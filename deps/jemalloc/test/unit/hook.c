...
static void alloc_free_size(size_t sz) {
...
static void set_args_raw(uintptr_t *args_raw, int nargs) {
    memcpy(arg_args_raw, args_raw, sizeof(uintptr_t) * nargs);
}

static void assert_args_raw(uintptr_t *args_raw_expected, int nargs) {
    int cmp = memcmp(args_raw_expected, arg_args_raw, sizeof(uintptr_t) * nargs);
...
static void test_alloc_hook(void *extra, hook_alloc_t type, void *result, uintptr_t result_raw, uintptr_t args_raw[3]) {
    call_count++;
    arg_extra = extra;
    arg_type = (int)type;
    arg_result = result;
...
static void test_dalloc_hook(void *extra, hook_dalloc_t type, void *address, uintptr_t args_raw[3]) {
    call_count++;
    arg_extra = extra;
    arg_type = (int)type;
    arg_address = address;
...
static void test_expand_hook(void *extra, hook_expand_t type, void *address, size_t old_usize, size_t new_usize, uintptr_t result_raw, uintptr_t args_raw[4]) {
    call_count++;
    arg_extra = extra;
    arg_type = (int)type;
    arg_address = address;
...
TEST_BEGIN(test_hooks_basic) {
    ...
    hooks_t   hooks = {&test_alloc_hook, &test_dalloc_hook, &test_expand_hook, (void *)111};
    ...
    assert_d_eq((int)hook_alloc_posix_memalign, arg_type, "Passed wrong alloc type");
    ...
    assert_d_eq((int)hook_dalloc_sdallocx, arg_type, "Passed wrong dalloc type");
    ...
    /* Expand */
    reset_args();
    hook_invoke_expand(hook_expand_xallocx, (void *)222, 333, 444, 555, args_raw);
    assert_d_eq((int)hook_expand_xallocx, arg_type, "Passed wrong expand type");
    assert_ptr_eq((void *)111, arg_extra, "Passed wrong user pointer");
    assert_ptr_eq((void *)222, arg_address, "Passed wrong address");
...
TEST_BEGIN(test_hooks_alloc_simple) {
    ...
    assert_u64_eq((uintptr_t)ptr, (uintptr_t)arg_result_raw, "Wrong raw result");
    ...
    assert_d_eq(call_count, 1, "Hook not called");
    assert_ptr_eq(arg_extra, (void *)123, "Wrong extra");
    assert_d_eq(arg_type, (int)hook_alloc_posix_memalign, "Wrong hook type");
    assert_ptr_eq(ptr, arg_result, "Wrong result");
    assert_u64_eq((uintptr_t)err, (uintptr_t)arg_result_raw, "Wrong raw result");
    ...
    assert_d_eq(call_count, 1, "Hook not called");
    assert_ptr_eq(arg_extra, (void *)123, "Wrong extra");
    assert_d_eq(arg_type, (int)hook_alloc_aligned_alloc, "Wrong hook type");
    assert_ptr_eq(ptr, arg_result, "Wrong result");
    assert_u64_eq((uintptr_t)ptr, (uintptr_t)arg_result_raw, "Wrong raw result");
    ...
    assert_u64_eq((uintptr_t)ptr, (uintptr_t)arg_result_raw, "Wrong raw result");
    assert_u64_eq((uintptr_t)1, arg_args_raw[0], "Wrong argument");
    assert_u64_eq((uintptr_t)MALLOCX_LG_ALIGN(10), arg_args_raw[1], "Wrong flags");
    free(ptr);
...
TEST_BEGIN(test_hooks_dalloc_simple) {
    ...
    assert_ptr_eq(ptr, arg_address, "Wrong pointer freed");
    assert_u64_eq((uintptr_t)ptr, arg_args_raw[0], "Wrong raw arg");
    assert_u64_eq((uintptr_t)MALLOCX_TCACHE_NONE, arg_args_raw[1], "Wrong raw arg");
    ...
    assert_u64_eq((uintptr_t)MALLOCX_TCACHE_NONE, arg_args_raw[2], "Wrong raw arg");
...
TEST_BEGIN(test_hooks_realloc_as_malloc_or_free) {
    hooks_t hooks = {&test_alloc_hook, &test_dalloc_hook, &test_expand_hook, (void *)123};
    void   *handle = hook_install(TSDN_NULL, &hooks);
    assert_ptr_ne(handle, NULL, "Hook installation failed");
...
static void do_realloc_test(void *(*ralloc)(void *, size_t, int), int flags, int expand_type, int dalloc_type) {
    hooks_t hooks = {&test_alloc_hook, &test_dalloc_hook, &test_expand_hook, (void *)123};
    void   *handle = hook_install(TSDN_NULL, &hooks);
    assert_ptr_ne(handle, NULL, "Hook installation failed");
    ...
    assert_u64_eq((uintptr_t)2 * 1024 * 1024, arg_args_raw[1], "Wrong argument");
    free(ptr);
    ...
    assert_u64_eq((uintptr_t)ptr2, (uintptr_t)arg_result_raw, "Wrong raw result");
    assert_u64_eq((uintptr_t)ptr, arg_args_raw[0], "Wrong argument");
...
static void *realloc_wrapper(void *ptr, size_t size, UNUSED int flags) {
    return realloc(ptr, size);
}

TEST_BEGIN(test_hooks_realloc) {
    do_realloc_test(&realloc_wrapper, 0, hook_expand_realloc, hook_dalloc_realloc);
}

TEST_END

TEST_BEGIN(test_hooks_rallocx) {
    do_realloc_test(&rallocx, MALLOCX_TCACHE_NONE, hook_expand_rallocx, hook_dalloc_rallocx);
}
...
int main(void) {
    ...
    return test_no_reentrancy(test_hooks_basic, test_hooks_null, test_hooks_remove, test_hooks_alloc_simple, test_hooks_dalloc_simple, test_hooks_expand_simple, test_hooks_realloc_as_malloc_or_free, test_hooks_realloc, test_hooks_rallocx);
}
