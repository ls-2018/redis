...
TEST_BEGIN(test_base_hooks_default) {
    ...
    if (config_stats) {
        ...
        assert_zu_ge(allocated0, sizeof(base_t), "Base header should count as allocated");
        if (opt_metadata_thp == metadata_thp_always) {
            assert_zu_gt(n_thp, 0, "Base should have 1 THP at least.");
        }
    }

    assert_ptr_not_null(base_alloc(tsdn, base, 42, 1), "Unexpected base_alloc() failure");

    if (config_stats) {
        base_stats_get(tsdn, base, &allocated1, &resident, &mapped, &n_thp);
        assert_zu_ge(allocated1 - allocated0, 42, "At least 42 bytes were allocated by base_alloc()");
    }
...
TEST_BEGIN(test_base_hooks_not_null) {
     ...
     */
    {
        const size_t alignments[] = {
            1, QUANTUM, QUANTUM << 1, CACHELINE, CACHELINE << 1,
        };
        unsigned i;
        ...
        for (i = 0; i < sizeof(alignments) / sizeof(size_t); i++) {
            ...
            assert_ptr_eq(p, (void *)(ALIGNMENT_CEILING((uintptr_t)p, alignment)), "Expected quantum alignment");
            q = base_alloc(tsdn, base, alignment, alignment);
            assert_ptr_not_null(q, "Unexpected base_alloc() failure");
            assert_ptr_eq((void *)((uintptr_t)p + align_ceil), q, "Minimal allocation should take up %zu bytes", align_ceil);
            r = base_alloc(tsdn, base, 1, alignment);
            assert_ptr_not_null(r, "Unexpected base_alloc() failure");
            assert_ptr_eq((void *)((uintptr_t)q + align_ceil), r, "Minimal allocation should take up %zu bytes", align_ceil);
        }
    }
     ...
     * allocation.
     */
    assert_zu_ge(extent_bsize_get(&base->blocks->extent), QUANTUM, "Remainder insufficient for test");
     ...
     */
    {
        ...
        for (i = 0; i < sizeof(alignments) / sizeof(size_t); i++) {
            ...
            assert_ptr_not_null(p, "Unexpected base_alloc() failure");
            assert_ptr_eq(p, (void *)(ALIGNMENT_CEILING((uintptr_t)p, alignment)), "Expected %zu-byte alignment", alignment);
        }
    }

    called_dalloc = called_destroy = called_decommit = called_purge_lazy = called_purge_forced = false;
    base_delete(tsdn, base);
...
int main(void) {
    return test(test_base_hooks_default, test_base_hooks_null, test_base_hooks_not_null);
}
