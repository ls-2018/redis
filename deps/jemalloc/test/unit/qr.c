...
static void test_entries_ring(ring_t *entries) {
    ...
    for (i = 0; i < NENTRIES; i++) {
        j = 0;
        qr_foreach(t, &entries[i], link) {
            assert_c_eq(t->id, entries[(i + j) % NENTRIES].id, "Element id mismatch");
            j++;
        }
    }
    for (i = 0; i < NENTRIES; i++) {
        j = 0;
        qr_reverse_foreach(t, &entries[i], link) {
            assert_c_eq(t->id, entries[(NENTRIES + i - j - 1) % NENTRIES].id, "Element id mismatch");
            j++;
        }
    }
    for (i = 0; i < NENTRIES; i++) {
        t = qr_next(&entries[i], link);
        assert_c_eq(t->id, entries[(i + 1) % NENTRIES].id, "Element id mismatch");
    }
    for (i = 0; i < NENTRIES; i++) {
        t = qr_prev(&entries[i], link);
        assert_c_eq(t->id, entries[(NENTRIES + i - 1) % NENTRIES].id, "Element id mismatch");
    }
}
...
TEST_BEGIN(test_qr_remove) {
    ...
    for (i = 0; i < NENTRIES; i++) {
        j = 0;
        qr_foreach(t, &entries[i], link) {
            assert_c_eq(t->id, entries[i + j].id, "Element id mismatch");
            j++;
        }
        j = 0;
        qr_reverse_foreach(t, &entries[i], link) {
            assert_c_eq(t->id, entries[NENTRIES - 1 - j].id, "Element id mismatch");
            j++;
        }
...
TEST_BEGIN(test_qr_before_insert) {
    ...
    for (i = 0; i < NENTRIES; i++) {
        j = 0;
        qr_foreach(t, &entries[i], link) {
            assert_c_eq(t->id, entries[(NENTRIES + i - j) % NENTRIES].id, "Element id mismatch");
            j++;
        }
    }
    for (i = 0; i < NENTRIES; i++) {
        j = 0;
        qr_reverse_foreach(t, &entries[i], link) {
            assert_c_eq(t->id, entries[(i + j + 1) % NENTRIES].id, "Element id mismatch");
            j++;
        }
    }
    for (i = 0; i < NENTRIES; i++) {
...
static void test_split_entries(ring_t *entries) {
    ...
    for (i = 0; i < NENTRIES; i++) {
        ...
        qr_foreach(t, &entries[i], link) {
            if (i < SPLIT_INDEX) {
                assert_c_eq(t->id, entries[(i + j) % SPLIT_INDEX].id, "Element id mismatch");
            }
            else {
                assert_c_eq(t->id, entries[(i + j - SPLIT_INDEX) % (NENTRIES - SPLIT_INDEX) + SPLIT_INDEX].id, "Element id mismatch");
            }
            j++;
        }
    }
...
int main(void) {
    return test(test_qr_one, test_qr_after_insert, test_qr_remove, test_qr_before_insert, test_qr_meld_split);
}
