...
static size_t do_get_size_impl(const char *cmd, unsigned arena_ind) {
    size_t mib[4];
    ...
    mib[2] = arena_ind;
    size_t size;
    assert_d_eq(mallctlbymib(mib, miblen, (void *)&size, &z, NULL, 0), 0, "Unexpected mallctlbymib([\"%s\"], ...) failure", cmd);

    return size;
}

static size_t do_get_active(unsigned arena_ind) {
    return do_get_size_impl("stats.arenas.0.pactive", arena_ind) * PAGE;
}

static size_t do_get_mapped(unsigned arena_ind) {
...
static void *thd_start(void *arg) {
    for (unsigned next_epoch = 1; next_epoch < NEPOCHS; next_epoch++) {
        ...
        while ((cur_epoch = atomic_load_u(&epoch, ATOMIC_ACQUIRE)) != next_epoch) {
...
TEST_BEGIN(test_retained) {
    ...
    for (unsigned e = 1; e < NEPOCHS; e++) {
        ...
        size_t   usable = 0;
        size_t   fragmented = 0;
        for (pszind_t pind = sz_psz2ind(HUGEPAGE); pind < arena->extent_grow_next; pind++) {
        ...
        assert_u_eq(do_arena_create(NULL), arena_ind, "Unexpected arena index");
    }
