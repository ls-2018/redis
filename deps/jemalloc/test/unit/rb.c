...
#define rbtn_black_height(a_type, a_field, a_rbt, r_height)                                                                         \
    do {                                                                                                                            \
        a_type *rbp_bh_t;                                                                                                           \
        for (rbp_bh_t = (a_rbt)->rbt_root, (r_height) = 0; rbp_bh_t != NULL; rbp_bh_t = rbtn_left_get(a_type, a_field, rbp_bh_t)) { \
...
rb_gen(
    static, tree_, tree_t, node_t, link, node_cmp
...
static unsigned tree_recurse(node_t *node, unsigned black_height, unsigned black_depth) {
    unsigned ret = 0;
    node_t  *left_node;
    node_t  *right_node;
    ...
    if (rbtn_red_get(node_t, link, node)) {
        if (left_node != NULL) {
            assert_false(rbtn_red_get(node_t, link, left_node), "Node should be black");
        }
        if (right_node != NULL) {
            assert_false(rbtn_red_get(node_t, link, right_node), "Node should be black");
        }
    }
...
static node_t *tree_iterate_cb(tree_t *tree, node_t *node, void *data) {
    ...
    assert_ptr_eq(search_node, node, "tree_search() returned unexpected node");

    /* Test rb_nsearch(). */
    search_node = tree_nsearch(tree, node);
    assert_ptr_eq(search_node, node, "tree_nsearch() returned unexpected node");

    /* Test rb_psearch(). */
    search_node = tree_psearch(tree, node);
    assert_ptr_eq(search_node, node, "tree_psearch() returned unexpected node");
...
static void node_remove(tree_t *tree, node_t *node, unsigned nnodes) {
    ...
    search_node = tree_nsearch(tree, node);
    if (search_node != NULL) {
        assert_u64_ge(search_node->key, node->key, "Key ordering error");
    }

    /* Test rb_psearch(). */
    search_node = tree_psearch(tree, node);
    if (search_node != NULL) {
        assert_u64_le(search_node->key, node->key, "Key ordering error");
    }
    ...
    assert_u_eq(tree_iterate(tree), nnodes - 1, "Unexpected node iteration count");
    assert_u_eq(tree_iterate_reverse(tree), nnodes - 1, "Unexpected node iteration count");
}
    ...
    for (i = 0; i < NBAGS; i++) {
        ...
        for (j = 1; j <= NNODES; j++) {
            ...
            for (k = 0; k < j; k++) {
                ...
                assert_u_eq(tree_iterate(&tree), k + 1, "Unexpected node iteration count");
                assert_u_eq(tree_iterate_reverse(&tree), k + 1, "Unexpected node iteration count");

                assert_false(tree_empty(&tree), "Tree should not be empty");
                assert_ptr_not_null(tree_first(&tree), "Tree should not be empty");
                assert_ptr_not_null(tree_last(&tree), "Tree should not be empty");
            ...
            switch (i % 5) {
                ...
                case 2: {
                    ...
                    start = NULL;
                    do {
                        start = tree_iter(&tree, start, remove_iterate_cb, (void *)&nnodes);
                        nnodes--;
                    } while (start != NULL);
                    assert_u_eq(nnodes, 0, "Removal terminated early");
                    break;
                }
                case 3: {
                    node_t  *start;
                    ...
                    start = NULL;
                    do {
                        start = tree_reverse_iter(&tree, start, remove_reverse_iterate_cb, (void *)&nnodes);
                        nnodes--;
                    } while (start != NULL);
                    assert_u_eq(nnodes, 0, "Removal terminated early");
                    break;
                ...
                case 4: {
                    ...
                    assert_u_eq(nnodes, 0, "Destruction terminated early");
                    break;
                }
                default:
                    not_reached();
