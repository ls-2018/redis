...
static void set_data(data_t *data, int num) {
    for (int i = 0; i < 10; i++) {
        data->arr[i] = num;
    }
}

static void assert_data(data_t *data) {
    int num = data->arr[0];
    for (int i = 0; i < 10; i++) {
...
static void *seq_reader_thd(void *arg) {
    ...
    while (iter < 1000 * 1000 - 1) {
        ...
        if (success) {
            ...
            assert_d_le(iter, local_data.arr[0], "Seq read went back in time.");
...
static void *seq_writer_thd(void *arg) {
    thd_data_t *thd_data = (thd_data_t *)arg;
    data_t      local_data;
