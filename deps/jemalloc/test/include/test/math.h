 ...
 */
static inline double ln_gamma(double x) {
    double f, z;
 ...
 */
static inline double i_gamma(double x, double p, double ln_gamma_p) {
 ...
 */
static inline double pt_norm(double p) {
    double q, r, ret;
 ...
 */
static inline double pt_chi2(double p, double df, double ln_gamma_df_2) {
    ...
    if (df < -1.24 * log(p)) {
        ...
        if (df > 0.32) {
            ...
            while (true) {
                ...
                ch -= (1.0 - exp(a + ln_gamma_df_2 + 0.5 * ch + c * aa) * p2 / p1) / t;
 ...
 * p.
 */
static inline double pt_gamma(double p, double shape, double scale, double ln_gamma_shape) {
