...
typedef struct sentinelRedisInstance {
    ...
    mstime_t s_down_since_time;               // 主管下线的时长
...
const char *preMonitorCfgName[] = {"announce-ip", "announce-port", "deny-scripts-reconfig", "sentinel-user", "sentinel-pass", "current-epoch", "myid", "resolve-hostnames", "announce-hostnames"};

// 这个函数用Sentinel特定的默认值覆盖一些普通的Redis配置默认值.
void initSentinelConfig(void) {
    server.port = REDIS_SENTINEL_PORT; // 26379
    server.protected_mode = 0;         // 哨兵必须暴露.
}
...
void sentinelEvent(int level, char *type, sentinelRedisInstance *ri, const char *fmt, ...) {
    ...
    // 判断监听实例的类型是否是主节点
    if (fmt[0] == '%' && fmt[1] == '@') {
        // 判断实例的flags标签是否为SRI_MASTER,如果有,代表是主节点
        sentinelRedisInstance *master = (ri->flags & SRI_MASTER) ? NULL : ri->master;
        // 如果是主节点, 根据实例名称、IP地址、端口号等信息拼接要传递的消息
        if (master) {
            snprintf(msg, sizeof(msg), "%s %s %s %d @ %s %s %d", sentinelRedisInstanceTypeStr(ri), ri->name, announceSentinelAddr(ri->addr), ri->addr->port, master->name, announceSentinelAddr(master->addr), master->addr->port);
        }
        else {
            snprintf(msg, sizeof(msg), "%s %s %s %d", sentinelRedisInstanceTypeStr(ri), ri->name, announceSentinelAddr(ri->addr), ri->addr->port);
        }
        fmt += 2;
    }
    else {
...
void sentinelGenerateInitialMonitorEvents(void) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        // 发送事件信息的 ,type 代表频道,ri对应交互的主节点,fmt表示发送的消息
...
void sentinelCallClientReconfScript(sentinelRedisInstance *master, int role, char *state, sentinelAddr *from, sentinelAddr *to) {
    ...
    sentinelScheduleScriptExecution(master->client_reconfig_script, master->name, (role == SENTINEL_LEADER) ? "leader" : "observer", state, announceSentinelAddr(from), fromport, announceSentinelAddr(to), toport, NULL);
}
...
sentinelRedisInstance *getSentinelRedisInstanceByAddrAndRunID(dict *instances, char *addr, int port, char *runid) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        if (runid && !ri->runid)
            continue;
        if ((runid == NULL || strcmp(ri->runid, runid) == 0) && (addr == NULL || sentinelAddrOrHostnameEqual(ri->addr, ri_addr))) {
            instance = ri;
            break;
        }
    }
...
int sentinelResetMasterAndChangeAddress(sentinelRedisInstance *master, char *hostname, int port) {
    ...
    for (j = 0; j < numslaves; j++) {
        ...
        slave = createSentinelRedisInstance(NULL, SRI_SLAVE, slaves[j]->hostname, slaves[j]->port, master->quorum, master);
...
sentinelAddr *sentinelGetCurrentMasterAddress(sentinelRedisInstance *master) {
     ...
     * address. */
    if ((master->flags & SRI_FAILOVER_IN_PROGRESS) && master->promoted_slave && master->failover_state >= SENTINEL_FAILOVER_STATE_RECONF_SLAVES) {
...
const char *sentinelHandleConfiguration(char **argv, int argc) {
    ...
    else if (!strcasecmp(argv[0], "down-after-milliseconds") && argc == 3) { // 主从库断接的最大连接超时时间
...
void rewriteConfigSentinelOption(struct rewriteConfigState *state) {
    ...
    rewriteConfigRewriteLine(state, "sentinel deny-scripts-reconfig", line, sentinel.deny_scripts_reconfig != SENTINEL_DEFAULT_DENY_SCRIPTS_RECONFIG);
    ...
    rewriteConfigRewriteLine(state, "sentinel resolve-hostnames", line, sentinel.resolve_hostnames != SENTINEL_DEFAULT_RESOLVE_HOSTNAMES);

    /* sentinel announce-hostnames. */
    line = sdscatprintf(sdsempty(), "sentinel announce-hostnames %s", sentinel.announce_hostnames ? "yes" : "no");
    rewriteConfigRewriteLine(state, "sentinel announce-hostnames", line, sentinel.announce_hostnames != SENTINEL_DEFAULT_ANNOUNCE_HOSTNAMES);
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        line = sdscatprintf(sdsempty(), "sentinel monitor %s %s %d %d", master->name, announceSentinelAddr(master_addr), master_addr->port, master->quorum);
        ...
        if (master->down_after_period != sentinel_default_down_after) {
            // 主从库断接的最大连接超时时间
            line = sdscatprintf(sdsempty(), "sentinel down-after-milliseconds %s %ld", master->name, (long)master->down_after_period);
            rewriteConfigRewriteLine(state, "sentinel down-after-milliseconds", line, 1); // 主从库断接的最大连接超时时间
            /* rewriteConfigMarkAsProcessed is handled after the loop */
        }

        /* sentinel failover-timeout */
        if (master->failover_timeout != sentinel_default_failover_timeout) {
            line = sdscatprintf(sdsempty(), "sentinel failover-timeout %s %ld", master->name, (long)master->failover_timeout);
        ...
        if (master->notification_script) {
            line = sdscatprintf(sdsempty(), "sentinel notification-script %s %s", master->name, master->notification_script);
        ...
        if (master->master_reboot_down_after_period != 0) {
            line = sdscatprintf(sdsempty(), "sentinel master-reboot-down-after-period %s %ld", master->name, (long)master->master_reboot_down_after_period);
            rewriteConfigRewriteLine(state, "sentinel master-reboot-down-after-period", line, 1);
            /* rewriteConfigMarkAsProcessed is handled after the loop */
        }

        /* sentinel config-epoch */
        line = sdscatprintf(sdsempty(), "sentinel config-epoch %s %llu", master->name, (unsigned long long)master->config_epoch);
        rewriteConfigRewriteLine(state, "sentinel config-epoch", line, 1);
        /* rewriteConfigMarkAsProcessed is handled after the loop */

        /* sentinel leader-epoch */
        line = sdscatprintf(sdsempty(), "sentinel leader-epoch %s %llu", master->name, (unsigned long long)master->leader_epoch);
        ...
        while ((de = dictNext(di2)) != NULL) {
            ...
            line = sdscatprintf(sdsempty(), "sentinel known-replica %s %s %d", master->name, announceSentinelAddr(slave_addr), slave_addr->port);
        ...
        while ((de = dictNext(di2)) != NULL) {
            ...
            if (ri->runid == NULL)
                continue;
            line = sdscatprintf(sdsempty(), "sentinel known-sentinel %s %s %d %s", master->name, announceSentinelAddr(ri->addr), ri->addr->port, ri->runid);
    ...
    */
    ...
    rewriteConfigMarkAsProcessed(state, "sentinel down-after-milliseconds"); // 主从库断接的最大连接超时时间
...
void sentinelSendAuthIfNeeded(sentinelRedisInstance *ri, redisAsyncContext *c) {
    ...
    if (auth_pass && auth_user == NULL) {
        if (redisAsyncCommand(c, sentinelDiscardReplyCallback, ri, "%s %s", sentinelInstanceMapCommand(ri, "AUTH"), auth_pass) == C_OK)
        ...
        if (redisAsyncCommand(c, sentinelDiscardReplyCallback, ri, "%s %s %s", sentinelInstanceMapCommand(ri, "AUTH"), auth_user, auth_pass) == C_OK)
            ri->link->pending_commands++;
    }
}
...
void sentinelSetClientName(sentinelRedisInstance *ri, redisAsyncContext *c, char *type) {
    ...
    if (redisAsyncCommand(c, sentinelDiscardReplyCallback, ri, "%s SETNAME %s", sentinelInstanceMapCommand(ri, "CLIENT"), name) == C_OK) {
        ri->link->pending_commands++;
    }
}
...
int sentinelMasterLooksSane(sentinelRedisInstance *master) {
    return master->flags & SRI_MASTER && master->role_reported == SRI_MASTER && (master->flags & (SRI_S_DOWN | SRI_O_DOWN)) == 0 && (mstime() - master->info_refresh) < sentinel_info_period * 2;
}
...
void sentinelRefreshInstanceInfo(sentinelRedisInstance *ri, const char *info) {
    ...
    for (j = 0; j < numlines; j++) {
        ...
        if (role == SRI_SLAVE) {
            ...
            if (sdslen(l) >= 19 && !memcmp(l, "master_link_status:", 19)) {
                ri->slave_master_link_status = (strcasecmp(l + 19, "up") == 0) ? SENTINEL_MASTER_LINK_STATUS_UP : SENTINEL_MASTER_LINK_STATUS_DOWN;
            }
    ...
    if ((ri->flags & SRI_SLAVE) && role == SRI_SLAVE && (ri->slave_master_port != ri->master->addr->port || !sentinelAddrEqualsHostname(ri->master->addr, ri->slave_master_host))) {
    ...
    if ((ri->flags & SRI_SLAVE) && role == SRI_SLAVE && (ri->flags & (SRI_RECONF_SENT | SRI_RECONF_INPROG))) {
        ...
        if ((ri->flags & SRI_RECONF_SENT) && ri->slave_master_host && sentinelAddrEqualsHostname(ri->master->promoted_slave->addr, ri->slave_master_host) && ri->slave_master_port == ri->master->promoted_slave->addr->port) {
    ...
    if (r->type == REDIS_REPLY_STATUS || r->type == REDIS_REPLY_ERROR) {
        ...
        if (strncmp(r->str, "PONG", 4) == 0 || strncmp(r->str, "LOADING", 7) == 0 || strncmp(r->str, "MASTERDOWN", 10) == 0) {
            ...
            if (strncmp(r->str, "BUSY", 4) == 0 && (ri->flags & SRI_S_DOWN) && !(ri->flags & SRI_SCRIPT_KILL_SENT)) {
                if (redisAsyncCommand(ri->link->cc, sentinelDiscardReplyCallback, ri, "%s KILL", sentinelInstanceMapCommand(ri, "SCRIPT")) == C_OK) {
                    ri->link->pending_commands++;
                }
...
void sentinelReceiveHelloMessages(redisAsyncContext *c, void *reply, void *privdata) {
    ...
    if (r->type != REDIS_REPLY_ARRAY || r->elements != 3 || r->element[0]->type != REDIS_REPLY_STRING || r->element[1]->type != REDIS_REPLY_STRING || r->element[2]->type != REDIS_REPLY_STRING || strcmp(r->element[0]->str, "message") != 0)
        return;
...
int sentinelSendHello(sentinelRedisInstance *ri) {
    ...
    snprintf(
        ...
        /* --- */
        master->name, announceSentinelAddr(master_addr), master_addr->port, (unsigned long long)master->config_epoch);
...
void addReplySentinelRedisInstance(client *c, sentinelRedisInstance *ri) {
    ...
    addReplyBulkCString(c, "down-after-milliseconds"); // 主从库断接的最大连接超时时间
...
void sentinelCommand(client *c) {
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "help")) {
        const char *help[] = {
            ...
            "    检查当前Sentinel配置是否能够达到主故障转移所需的quorum，以及授权故障转移所需的majority.",
         ...
         */
        if (anetResolve(NULL, c->argv[3]->ptr, ip, sizeof(ip), sentinel.resolve_hostnames ? ANET_NONE : ANET_IP_ONLY) == ANET_ERR) {
    ...
    if (sec_all || (dictFind(sections_dict, "sentinel") != NULL)) {
        ...
        while ((de = dictNext(di)) != NULL) {
            ...
            info = sdscatprintf(
                ...
                master_id++, ri->name, status, announceSentinelAddr(ri->addr), ri->addr->port, dictSize(ri->slaves), dictSize(ri->sentinels) + 1);
        }
        dictReleaseIterator(di);
    }
...
void sentinelSetCommand(client *c) {
    ...
    for (j = 3; j < c->argc; j++) {
        ...
        switch (numargs) {
            case 2:
                sentinelEvent(LL_WARNING, "+set", ri, "%@ %s %s", (char *)c->argv[old_j]->ptr, redacted ? "******" : (char *)c->argv[old_j + 1]->ptr);
                break;
            case 3:
                sentinelEvent(LL_WARNING, "+set", ri, "%@ %s %s %s", (char *)c->argv[old_j]->ptr, (char *)c->argv[old_j + 1]->ptr, (char *)c->argv[old_j + 2]->ptr);
                break;
            default:
...
void sentinelCheckSubjectivelyDown(sentinelRedisInstance *ri) {
    ...
    if (ri->link->cc && (mstime() - ri->link->cc_conn_time) > sentinel_min_link_reconnect_period && ri->link->act_ping_time != 0 && /* There is a pending ping... */
        /* The pending ping is delayed, and we did not receive
         * error replies as well. */
        (mstime() - ri->link->act_ping_time) > (ri->down_after_period / 2) && (mstime() - ri->link->last_pong_time) > (ri->down_after_period / 2)) {
        instanceLinkCloseConnection(ri->link, ri->link->cc);
    }
     ...
     */
    if (ri->link->pc && (mstime() - ri->link->pc_conn_time) > sentinel_min_link_reconnect_period && (mstime() - ri->link->pc_last_activity) > (sentinel_publish_period * 3)) {
        instanceLinkCloseConnection(ri->link, ri->link->pc);
    }
    ...
    if (elapsed > ri->down_after_period || (ri->flags & SRI_MASTER && ri->role_reported == SRI_SLAVE && mstime() - ri->role_reported_time > (ri->down_after_period + sentinel_info_period * 2)) || (ri->flags & SRI_MASTER_REBOOT && mstime() - ri->master_reboot_since_time > ri->master_reboot_down_after_period)) {
...
void sentinelReceiveIsMasterDownReply(redisAsyncContext *c, void *reply, void *privdata) {
    ...
    if (r->type == REDIS_REPLY_ARRAY && r->elements == 3 && r->element[0]->type == REDIS_REPLY_INTEGER && r->element[1]->type == REDIS_REPLY_STRING && r->element[2]->type == REDIS_REPLY_INTEGER) {
        ri->last_master_down_reply_time = mstime();
...
void sentinelAskMasterStateToOtherSentinels(sentinelRedisInstance *master, int flags) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        retval = redisAsyncCommand(ri->link->cc, sentinelReceiveIsMasterDownReply, ri, "%s is-master-down-by-addr %s %s %llu %s", sentinelInstanceMapCommand(ri, "SENTINEL"), announceSentinelAddr(master->addr), port, sentinel.current_epoch, (master->failover_state > SENTINEL_FAILOVER_STATE_NONE) ? sentinel.myid : "*");
        if (retval == C_OK)
            ri->link->pending_commands++;
    }
    dictReleaseIterator(di);
...
int sentinelSendSlaveOf(sentinelRedisInstance *ri, const sentinelAddr *addr) {
    ...
    for (int type = 0; type < 2; type++) {
        retval = redisAsyncCommand(ri->link->cc, sentinelDiscardReplyCallback, ri, "%s KILL TYPE %s", sentinelInstanceMapCommand(ri, "CLIENT"), type == 0 ? "normal" : "pubsub");
        if (retval == C_ERR)
            return retval;
        ri->link->pending_commands++;
    }
...
void sentinelFailoverSwitchToPromotedSlave(sentinelRedisInstance *master) {
    ...
    sentinelEvent(LL_WARNING, "+switch-master", master, "%s %s %d %s %d", master->name, announceSentinelAddr(master->addr), master->addr->port, announceSentinelAddr(ref->addr), ref->addr->port);
...
void sentinelHandleDictOfRedisInstances(dict *instances) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        if (ri->flags & SRI_MASTER) {                          // 如果当前节点是主节点,那么调用sentinelHandleDictOfRedisInstances对它的所有从节点进行处理.
            sentinelHandleDictOfRedisInstances(ri->slaves);    // 处理该master节点对应的slave节点
            sentinelHandleDictOfRedisInstances(ri->sentinels); // 处理该master节点被监控的哨兵节点
