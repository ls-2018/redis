...
void *rdbGenericLoadStringObject(rio *rdb, int flags, size_t *lenptr) {
    ...
    if (plain || sds) {
        ...
        robj *o = encode ? tryCreateStringObject(SDS_NOINIT, len) : tryCreateRawStringObject(SDS_NOINIT, len);
        if (!o) {
            serverLog(isRestoreContext() ? LL_VERBOSE : LL_WARNING, "rdbGenericLoadStringObject failed allocating %llu bytes", len);
            return NULL;
        }
...
ssize_t rdbSaveAuxField(rio *rdb, void *key, size_t keylen, void *val, size_t vallen) {
    ...
    if ((ret = rdbSaveType(rdb, RDB_OPCODE_AUX)) == -1) // 写入操作码
        return -1;
    len += ret;
    if ((ret = rdbSaveRawString(rdb, key, keylen)) == -1) // 写入属性信息中的键
        return -1;
    len += ret;
    if ((ret = rdbSaveRawString(rdb, val, vallen)) == -1) // 写入属性信息中的值
        return -1;
    len += ret;
    return len;
}
...
int rdbSaveInfoAuxFields(rio *rdb, int rdbflags, rdbSaveInfo *rsi) {
    ...
    if (rdbSaveAuxFieldStrInt(rdb, "redis-bits", redis_bits) == -1) // 运行平台的架构信息
        return -1;
    if (rdbSaveAuxFieldStrInt(rdb, "ctime", time(NULL)) == -1) // rdb文件的创建时间
        return -1;
...
int rdbSave(int req, char *filename, rdbSaveInfo *rsi) {
    ...
    if (rename(tmpfile, filename) == -1) {
        char *str_err = strerror(errno);
        char *cwdp = getcwd(cwd, MAXPATHLEN);
        serverLog(LL_WARNING, "Error 移动临时文件 %s 到 %s (在server的根目录: %s): %s", tmpfile, filename, cwdp ? cwdp : "unknown", str_err);
        unlink(tmpfile);
        stopSaving(0);
        return C_ERR;
    }

    serverLog(LL_NOTICE, "db文件已保存至硬盘");
    server.dirty = 0;
...
werr:
    serverLog(LL_WARNING, "db文件已保存至硬盘出错: %s", strerror(errno));
    if (fp)
        fclose(fp);
    unlink(tmpfile);
    stopSaving(0);
...
int rdbSaveBackground(int req, char *filename, rdbSaveInfo *rsi) {
    ...
    server.dirty_before_bgsave = server.dirty; // 上一次 rdb 开始到本次rdb开始时 键值对的变化次数
    server.lastbgsave_try = time(NULL);        // 将 上一次尝试dbsave的时间设置为当前时间戳

    if ((childpid = redisFork(CHILD_TYPE_RDB)) == 0) { // 子进程
        int retval;

        redisSetProcTitle("redis-rdb-bgsave"); // 设置子进程的名称
        redisSetCpuAffinity(server.bgsave_cpulist);
        retval = rdbSave(req, filename, rsi);
        if (retval == C_OK) { // rdb 文件完成保存之后 对 rdb 过程中的资源消耗进行计算
...
robj *rdbLoadObject(int rdbtype, rio *rdb, sds key, int dbid, int *error) {
    ...
    if (rdbtype == RDB_TYPE_STRING) {
        ...
        if (zsetLength(o) <= server.zset_max_listpack_entries && maxelelen <= server.zset_max_listpack_value && lpSafeToAdd(NULL, totelelen)) {
        ...
        while (o->encoding == OBJ_ENCODING_LISTPACK && len > 0) {
            ...
            if (sdslen(field) > server.hash_max_listpack_value || sdslen(value) > server.hash_max_listpack_value || !lpSafeToAdd(o->ptr, sdslen(field) + sdslen(value))) {
                hashTypeConvert(o, OBJ_ENCODING_HT);
            ...
            case RDB_TYPE_HASH_ZIPMAP:
                 ...
                 * when loading dumps created by Redis 2.4 gets deprecated. */
                {
                    ...
                    while ((zi = zipmapNext(zi, &fstr, &flen, &vstr, &vlen)) != NULL) {
                        ...
                        if (!field || dictAdd(dupSearchDict, field, NULL) != DICT_OK || !lpSafeToAdd(lp, (size_t)flen + vlen)) {
        ...
        if (io.ver == 2) {
            ...
            if (eof != RDB_MODULE_OPCODE_EOF) {
                rdbReportCorruptRDB(
                    "The RDB file contains module data for the module '%s' that is not terminated by "
...
void stopLoading(int success) {
    ...
    moduleFireServerEvent(REDISMODULE_EVENT_LOADING, success ? REDISMODULE_SUBEVENT_LOADING_ENDED : REDISMODULE_SUBEVENT_LOADING_FAILED, NULL);
}
...
void stopSaving(int success) {
    ...
    moduleFireServerEvent(REDISMODULE_EVENT_PERSISTENCE, success ? REDISMODULE_SUBEVENT_PERSISTENCE_ENDED : REDISMODULE_SUBEVENT_PERSISTENCE_FAILED, NULL);
}
...
int rdbLoadRioWithLoadingCtx(rio *rdb, int rdbflags, rdbSaveInfo *rsi, rdbLoadingCtx *rdb_loading_ctx) {
    ...
    while (1) {
        ...
        if (type == RDB_OPCODE_EXPIRETIME) {
            ...
            else if (!strcasecmp(auxkey->ptr, "lua")) { /* Won't load the script back in memory anymore. */
            }
            else if (!strcasecmp(auxkey->ptr, "redis-ver")) {
                serverLog(LL_NOTICE, "加载%s版本的RDB", (char *)auxval->ptr);
            }
            ...
            else if (!strcasecmp(auxkey->ptr, "redis-bits")) { /* Just ignored. */
            }
            else {
...
void bgsaveCommand(client *c) {
    ...
    // 不能重复执行 BGSAVE
    if (server.child_type == CHILD_TYPE_RDB) {
        addReplyError(c, "不能重复执行 BGSAVE");
    }
    else if (hasActiveChildProcess() || server.in_exec) { // 不能在 BGREWRITEAOF 正在运行时执行
