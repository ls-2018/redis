    ...
    while (fgets(line, maxline, fp) != NULL) {
        ...
        *p = '\0';
        ...
        while (p) {
            ...
            else if (!strcasecmp(s, "noflags")) { /* nothing to do */
            }
            else {
...
int clusterSaveConfig(int do_fsync) {
    ...
    ci = sdscatprintf(ci, "vars currentEpoch %llu lastVoteEpoch %llu\n", (unsigned long long)server.cluster->currentEpoch, (unsigned long long)server.cluster->lastVoteEpoch);
     ...
     * is just garbage. */
    if (content_size != sdslen(ci) && ftruncate(fd, content_size) == -1) { /* ftruncate() failing is not a critical error. */
    }
    close(fd);
    sdsfree(ci);
    return 0;
...
void clusterInit(void) {
    ...
    /* Lock the cluster config file to make sure every node uses its own nodes.conf. */
    ...
    if (!server.cluster_port && port > (65535 - CLUSTER_PORT_INCR)) {
        serverLog(LL_WARNING, "Redis端口号过高.集群通信端口为10000端口,比你的Redis端口高.你的Redis端口号必须小于等于55535.");
        exit(1);
    }
...
void setClusterNodeToInboundClusterLink(clusterNode *node, clusterLink *link) {
    ...
    if (node->inbound_link) {
        ...
        serverLog(LL_DEBUG, "Replacing inbound link fd %d from node %.40s with fd %d", node->inbound_link->conn->fd, node->name, link->conn->fd);
    }
...
void clusterAcceptHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while (max--) {
        ...
        connection *conn = server.tls_cluster ? connCreateAcceptedTLS(cfd, TLS_CLIENT_AUTH_YES) : connCreateAcceptedSocket(cfd);
...
int clusterBumpConfigEpochWithoutConsensus(void) {
    ...
    if (myself->configEpoch == 0 || myself->configEpoch != maxEpoch) {
        ...
        serverLog(LL_WARNING, "New configEpoch set to %llu", (unsigned long long)myself->configEpoch);
        return C_OK;
    }
    else {
        return C_ERR;
...
void clearNodeFailureIfNeeded(clusterNode *node) {
    ...
    if (nodeIsSlave(node) || node->numslots == 0) {
        serverLog(LL_NOTICE, "Clear FAIL state for node %.40s: %s is reachable again.", node->name, nodeIsSlave(node) ? "replica" : "master without slots");
        node->flags &= ~CLUSTER_NODE_FAIL;
        clusterDoBeforeSleep(CLUSTER_TODO_UPDATE_STATE | CLUSTER_TODO_SAVE_CONFIG);
    }
    ...
    if (nodeIsMaster(node) && node->numslots > 0 && (now - node->fail_time) > (server.cluster_node_timeout * CLUSTER_FAIL_UNDO_TIME_MULT)) {
        serverLog(LL_NOTICE, "Clear FAIL state for node %.40s: is reachable again and nobody is serving its slots after some time.", node->name);
        node->flags &= ~CLUSTER_NODE_FAIL;
        clusterDoBeforeSleep(CLUSTER_TODO_UPDATE_STATE | CLUSTER_TODO_SAVE_CONFIG);
    }
}
...
void clusterProcessGossipSection(clusterMsg *hdr, clusterLink *link) {
    ...
    while (count--) {
        ...
        if (node) {
            ...
            if (sender && nodeIsMaster(sender) && node != myself) {
                if (flags & (CLUSTER_NODE_FAIL | CLUSTER_NODE_PFAIL)) {
                    if (clusterNodeAddFailureReport(node, sender)) {
                        serverLog(LL_VERBOSE, "Node %.40s reported node %.40s as not reachable.", sender->name, node->name);
                    }
                    markNodeAsFailingIfNeeded(node);
                }
                else {
                    if (clusterNodeDelFailureReport(node, sender)) {
                        serverLog(LL_VERBOSE, "Node %.40s reported node %.40s is back online.", sender->name, node->name);
                    }
                }
            }
            ...
            if (!(flags & (CLUSTER_NODE_FAIL | CLUSTER_NODE_PFAIL)) && node->ping_sent == 0 && clusterNodeFailureReportsCount(node) == 0) {
             ...
             * new address. */
            if (node->flags & (CLUSTER_NODE_FAIL | CLUSTER_NODE_PFAIL) && !(flags & CLUSTER_NODE_NOADDR) && !(flags & (CLUSTER_NODE_FAIL | CLUSTER_NODE_PFAIL)) && (strcasecmp(node->ip, g->ip) || node->port != ntohs(g->port) || node->cport != ntohs(g->cport))) {
                if (node->link)
...
int nodeUpdateAddressIfNeeded(clusterNode *node, clusterLink *link, clusterMsg *hdr) {
    ...
    if (node->port == port && node->cport == cport && node->pport == pport && strcmp(ip, node->ip) == 0)
        return 0;
    ...
    if (node->link)
        freeClusterLink(node->link);
    node->flags &= ~CLUSTER_NODE_NOADDR;
    serverLog(LL_WARNING, "Address updated for node %.40s, now %s:%d", node->name, node->ip, node->port);
...
void clusterUpdateSlotsConfigWith(clusterNode *sender, uint64_t senderConfigEpoch, unsigned char *slots) {
    ...
    for (j = 0; j < CLUSTER_SLOTS; j++) {
        if (bitmapTestBit(slots, j)) {
            ...
            if (server.cluster->slots[j] == NULL || server.cluster->slots[j]->configEpoch < senderConfigEpoch) {
    ...
    if (newmaster && curmaster->numslots == 0 && (server.cluster_allow_replica_migration || sender_slots == migrated_our_slots)) {
        serverLog(
            LL_WARNING,
        ...
        serverLog(LL_WARNING, "I'm a sub-replica! Reconfiguring myself as a replica of grandmaster %.40s", myself->slaveof->slaveof->name);
...
int clusterProcessPacket(clusterLink *link) {
    ...
    serverLog(LL_DEBUG, "--- Processing packet of type %s, %lu bytes", clusterGetMessageTypeString(type), (unsigned long)totlen);
    ...
    if (type == CLUSTERMSG_TYPE_PING || type == CLUSTERMSG_TYPE_PONG || type == CLUSTERMSG_TYPE_MEET) {
        ...
        if (hdr->mflags[0] & CLUSTERMSG_FLAG0_EXT_DATA) {
            ...
            while (extensions--) {
                ...
                if (extlen % 8 != 0) {
                    serverLog(LL_WARNING, "Received a %s packet without proper padding (%d bytes)", clusterGetMessageTypeString(type), (int)extlen);
                    return 1;
                }
        ...
        explen = sizeof(clusterMsg) - sizeof(union clusterMsgData);
        explen += sizeof(clusterMsgDataPublish) - 8 + ntohl(hdr->data.publish.msg.channel_len) + ntohl(hdr->data.publish.msg.message_len);
    }
    ...
    if (totlen != explen) {
        serverLog(LL_WARNING, "Received invalid %s packet of length %lld but expected length %lld", clusterGetMessageTypeString(type), (unsigned long long)totlen, (unsigned long long)explen);
        return 1;
    }
    ...
    if (sender && !nodeInHandshake(sender)) {
        ...
        if (server.cluster->mf_end && nodeIsSlave(myself) && myself->slaveof == sender && hdr->mflags[0] & CLUSTERMSG_FLAG0_PAUSED && server.cluster->mf_master_offset == -1) {
    ...
    if (type == CLUSTERMSG_TYPE_PING || type == CLUSTERMSG_TYPE_MEET) {
        ...
        if ((type == CLUSTERMSG_TYPE_MEET || myself->ip[0] == '\0') && server.cluster_announce_ip == NULL) {
            ...
            if (connSockName(link->conn, ip, sizeof(ip), NULL) != -1 && strcmp(ip, myself->ip)) {
                memcpy(myself->ip, ip, NET_IP_STR_LEN);
                serverLog(LL_WARNING, "IP address for this node updated to %s", myself->ip);
    ...
    if (type == CLUSTERMSG_TYPE_PING || type == CLUSTERMSG_TYPE_PONG || type == CLUSTERMSG_TYPE_MEET) {
        serverLog(LL_DEBUG, "%s packet received: %.40s", clusterGetMessageTypeString(type), link->node ? link->node->name : "NULL");
        if (!link->inbound) {
            if (nodeInHandshake(link->node)) {
                 ...
                 * address. */
                serverLog(LL_DEBUG, "PONG contains mismatching sender ID. About node %.40s added %d ms ago, having flags %d", link->node->name, (int)(now - (link->node->ctime)), link->node->flags);
        ...
        if (sender && type == CLUSTERMSG_TYPE_PING && !nodeInHandshake(sender) && nodeUpdateAddressIfNeeded(sender, link, hdr)) {
            clusterDoBeforeSleep(CLUSTER_TODO_SAVE_CONFIG | CLUSTER_TODO_UPDATE_STATE);
        }
        ...
        if (sender) {
            ...
            if (failing && !(failing->flags & (CLUSTER_NODE_FAIL | CLUSTER_NODE_MYSELF))) {
                serverLog(LL_NOTICE, "FAIL message received from %.40s about %.40s", hdr->sender, hdr->data.fail.about.nodename);
            ...
            serverLog(LL_NOTICE, "Ignoring FAIL message from unknown node %.40s about %.40s", hdr->sender, hdr->data.fail.about.nodename);
        }
    }
    else if (type == CLUSTERMSG_TYPE_PUBLISH || type == CLUSTERMSG_TYPE_PUBLISHSHARD) {
        if (!sender)
         ...
         * Pub/Sub subscribers. */
        if ((type == CLUSTERMSG_TYPE_PUBLISH && serverPubsubSubscriptionCount() > 0) || (type == CLUSTERMSG_TYPE_PUBLISHSHARD && serverPubsubShardSubscriptionCount() > 0)) {
        ...
        pauseClients(PAUSE_DURING_FAILOVER, now + (CLUSTER_MF_TIMEOUT * CLUSTER_MF_PAUSE_MULT), CLIENT_PAUSE_WRITE);
        serverLog(LL_WARNING, "Manual failover requested by replica %.40s.", sender->name);
...
void clusterWriteHandler(connection *conn) {
    ...
    if (nwritten <= 0) {
        serverLog(LL_DEBUG, "I/O error writing to node link: %s", (nwritten == -1) ? connGetLastError(conn) : "short write");
...
void clusterLinkConnectHandler(connection *conn) {
    ...
    if (connGetState(conn) != CONN_STATE_CONNECTED) {
        serverLog(LL_VERBOSE, "Connection with Node %.40s at %s:%d failed: %s", node->name, node->ip, node->cport, connGetLastError(conn));
        freeClusterLink(link);
        return;
    }
...
void clusterReadHandler(connection *conn) {
    ...
    while (1) { /* Read as long as there is data to read. */
        ...
        if (nread <= 0) {
            /* I/O error... */
            serverLog(LL_DEBUG, "I/O error reading from node link: %s", (nread == 0) ? "connection closed" : connGetLastError(conn));
            handleLinkIOError(link);
            return;
        }
        else {
...
int clusterNodeIsInGossipSection(clusterMsg *hdr, int count, clusterNode *n) {
    int j;
    for (j = 0; j < count; j++) {
        if (memcmp(hdr->data.ping.gossip[j].nodename, n->name, CLUSTER_NAMELEN) == 0)
            break;
    }
    return j != count;
}
...
void clusterSetGossipEntry(clusterMsg *hdr, int i, clusterNode *n) {
    ...
    gossip = &(hdr->data.ping.gossip[i]);                   // 指向 gossip 信息结构
    memcpy(gossip->nodename, n->name, CLUSTER_NAMELEN);     // 将被选中节点的名字记录到 gossip 信息
    gossip->ping_sent = htonl(n->ping_sent / 1000);         // 将被选中节点的 PING 命令发送时间戳记录到 gossip 信息
    gossip->pong_received = htonl(n->pong_received / 1000); // 将被选中节点的 PONG 命令回复的时间戳记录到 gossip 信息
    memcpy(gossip->ip, n->ip, sizeof(n->ip));               // 将被选中节点的 IP 记录到 gossip 信息
    gossip->port = htons(n->port);                          // 将被选中节点的端口号记录到 gossip 信息
    gossip->cport = htons(n->cport);                        //
    gossip->flags = htons(n->flags);                        // 将被选中节点的标识值记录到 gossip 信息
...
void clusterSendPing(clusterLink *link, int type) {
    ...
    while (freshnodes > 0 && gossipcount < wanted && maxiterations--) {
         ...
         */
        if (this->flags & (CLUSTER_NODE_HANDSHAKE | CLUSTER_NODE_NOADDR) || (this->link == NULL && this->numslots == 0)) {
        ...
        // 如果是的话说明这个节点之前已经被选中了
        // 不要再选中它（否则就会出现重复）
        if (clusterNodeIsInGossipSection(hdr, gossipcount, this))
            continue;
    ...
    hdr->count = htons(gossipcount);                             // 将被选中节点的数量（gossip 信息中包含了多少个节点的信息）记录在 count 属性里面
    hdr->extensions = htons(extensions);                         //
    hdr->totlen = htonl(totlen);                                 // 将信息的长度记录到信息里面
    clusterSendMessage(link, buf, totlen);                       // 发送网络请求
    zfree(buf);
}
...
void clusterBroadcastPong(int target) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        if (target == CLUSTER_BROADCAST_LOCAL_SLAVES) {
            int local_slave = nodeIsSlave(node) && node->slaveof && (node->slaveof == myself || node->slaveof == myself->slaveof);
            if (!local_slave)
                continue;
        }
...
void clusterSendFailoverAuthIfNeeded(clusterNode *node, clusterMsg *request) {
    ...
    if (requestCurrentEpoch < server.cluster->currentEpoch) {
        serverLog(LL_WARNING, "Failover auth denied to %.40s: reqEpoch (%llu) < curEpoch(%llu)", node->name, (unsigned long long)requestCurrentEpoch, (unsigned long long)server.cluster->currentEpoch);
        return;
    }

    /* I already voted for this epoch? Return ASAP. */
    if (server.cluster->lastVoteEpoch == server.cluster->currentEpoch) {
        serverLog(LL_WARNING, "Failover auth denied to %.40s: already voted for epoch %llu", node->name, (unsigned long long)server.cluster->currentEpoch);
        return;
    }
    ...
    if (nodeIsMaster(node) || master == NULL || (!nodeFailed(master) && !force_ack)) {
        if (nodeIsMaster(node)) {
            serverLog(LL_WARNING, "Failover auth denied to %.40s: it is a master node", node->name);
        }
        else if (master == NULL) {
            serverLog(LL_WARNING, "Failover auth denied to %.40s: I don't know its master", node->name);
        }
        else if (!nodeFailed(master)) {
            serverLog(LL_WARNING, "Failover auth denied to %.40s: its master is up", node->name);
        }
        return;
    }
    ...
    for (j = 0; j < CLUSTER_SLOTS; j++) {
        ...
        serverLog(
            ...
            node->name, j, (unsigned long long)server.cluster->slots[j]->configEpoch, (unsigned long long)requestConfigEpoch);
        return;
    }
    ...
    serverLog(LL_WARNING, "Failover auth granted to %.40s for epoch %llu", node->name, (unsigned long long)server.cluster->currentEpoch);
}
...
int clusterGetSlaveRank(void) {
    ...
    for (j = 0; j < master->numslaves; j++)
        if (master->slaves[j] != myself && !nodeCantFailover(master->slaves[j]) && master->slaves[j]->repl_offset > myoffset)
            rank++;
    return rank;
}
...
void clusterLogCantFailover(int reason) {
    ...
    if (reason == server.cluster->cant_failover_reason && time(NULL) - lastlog_time < CLUSTER_CANT_FAILOVER_RELOG_PERIOD)
        return;
...
void clusterHandleSlaveFailover(void) {
    ...
    if (nodeIsMaster(myself) || myself->slaveof == NULL || (!nodeFailed(myself->slaveof) && !manual_failover) || (server.cluster_slave_no_failover && !manual_failover) || myself->slaveof->numslots == 0) {
    ...
    if (server.cluster_slave_validity_factor && data_age > (((mstime_t)server.repl_ping_slave_period * 1000) + (server.cluster_node_timeout * server.cluster_slave_validity_factor))) {
        if (!manual_failover) {
    ...
    if (auth_age > auth_retry_time) {
        server.cluster->failover_auth_time = mstime() + 500 + /* Fixed delay of 500 milliseconds, let FAIL msg propagate. */
        ...
        serverLog(
            ...
            "(rank #%d, offset %lld).",
            server.cluster->failover_auth_time - mstime(), server.cluster->failover_auth_rank, replicationGetSlaveOffset());
    ...
    if (server.cluster->failover_auth_sent == 0 && server.cluster->mf_end == 0) {
        ...
        if (newrank > server.cluster->failover_auth_rank) {
            ...
            serverLog(LL_WARNING, "Replica rank updated to #%d, added %lld milliseconds of delay.", newrank, added_delay);
        }
    }
    ...
    if (server.cluster->failover_auth_sent == 0) {
        ...
        serverLog(LL_WARNING, "Starting a failover election for epoch %llu.", (unsigned long long)server.cluster->currentEpoch);
    ...
    if (server.cluster->failover_auth_count >= needed_quorum) {
        ...
        if (myself->configEpoch < server.cluster->failover_auth_epoch) {
            ...
            serverLog(LL_WARNING, "configEpoch set to %llu after successful failover", (unsigned long long)myself->configEpoch);
        }
...
void clusterHandleSlaveMigration(int max_slaves) {
    ...
    if (target && candidate == myself && (mstime() - target->orphaned_time) > CLUSTER_SLAVE_MIGRATION_DELAY && !(server.cluster_module_flags & CLUSTER_MODULE_FLAG_NO_FAILOVER)) {
...
static int clusterNodeCronHandleReconnect(clusterNode *node, mstime_t handshake_timeout, mstime_t now) {
    ...
    if (node->link == NULL) {
        ...
        if (connConnect(link->conn, node->ip, node->cport, server.bind_source_addr, clusterLinkConnectHandler) == -1) {
...
void clusterCron(void) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        if (node->link &&                                            /* is connected */
            ...
            ping_delay > server.cluster_node_timeout / 2 && // 已经发过ping消息
...
void clusterUpdateState(void) {
    ...
    if (nodeIsMaster(myself) && server.cluster->state == CLUSTER_FAIL && mstime() - first_call_time < CLUSTER_WRITABLE_DELAY)
        return;
    ...
    if (new_state != server.cluster->state) {
        ...
        serverLog(LL_WARNING, "Cluster state changed: %s", new_state == CLUSTER_OK ? "ok" : "fail");
        server.cluster->state = new_state;
    }
}

// 检查当前节点的节点配置是否正确,包含的数据是否正确,在启动集群时被调用
int verifyClusterConfigWithData(void) {
    ...
    if (update_config)
        clusterSaveConfigOrDie(1); // 保存 nodes.conf 文件
    return C_OK;
}
...
static struct redisNodeFlags redisNodeFlagsTable[] = {{CLUSTER_NODE_MYSELF, "myself,"}, {CLUSTER_NODE_MASTER, "master,"}, {CLUSTER_NODE_SLAVE, "slave,"}, {CLUSTER_NODE_PFAIL, "fail?,"}, {CLUSTER_NODE_FAIL, "fail,"}, {CLUSTER_NODE_HANDSHAKE, "handshake,"}, {CLUSTER_NODE_NOADDR, "noaddr,"}, {CLUSTER_NODE_NOFAILOVER, "nofailover,"}};
...
sds clusterGenNodeDescription(clusterNode *node, int use_pport) {
    ...
    if (sdslen(node->hostname) != 0) {
        ci = sdscatfmt(ci, " %s:%i@%i,%s ", node->ip, port, node->cport, node->hostname);
    }
    else {
    ...
    ci = sdscatfmt(ci, " %I %I %U %s", (long long)node->ping_sent, (long long)node->pong_received, nodeEpoch, (node->link || node->flags & CLUSTER_NODE_MYSELF) ? "connected" : "disconnected");
...
void addNodeToNodeReply(client *c, clusterNode *node) {
    ...
    int use_pport = (server.tls_cluster && c->conn && connGetType(c->conn) != CONN_TYPE_TLS);
...
void addShardReplyForClusterShards(client *c, clusterNode *node, uint16_t *slot_info_pairs, int slot_pairs_count) {
    ...
    if (slot_info_pairs) {
        ...
        for (int i = 0; i < slot_pairs_count; i++) addReplyBulkLongLong(c, (unsigned long)slot_info_pairs[i]);
    }
    else {
...
void clusterCommand(client *c) {
    ...
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "help")) {
    ...
    else if ((!strcasecmp(c->argv[1]->ptr, "addslotsrange") || !strcasecmp(c->argv[1]->ptr, "delslotsrange")) && c->argc >= 4) {
        if (c->argc % 2 == 1) {
        ...
        sds reply = sdscatprintf(sdsempty(), "+%s %llu\r\n", (retval == C_OK) ? "BUMPED" : "STILL", (unsigned long long)myself->configEpoch);
        addReplySds(c, reply);
    }
        ...
        sds info = sdscatprintf(
            ...
            statestr[server.cluster->state], slots_assigned, slots_ok, slots_pfail, slots_fail, dictSize(server.cluster->nodes), server.cluster->size, (unsigned long long)server.cluster->currentEpoch, (unsigned long long)myepoch);
        ...
        for (int i = 0; i < CLUSTERMSG_TYPE_COUNT; i++) {
            ...
            info = sdscatprintf(info, "cluster_stats_messages_%s_sent:%lld\r\n", clusterGetMessageTypeString(i), server.cluster->stats_bus_messages_sent[i]);
        }
        ...
        for (int i = 0; i < CLUSTERMSG_TYPE_COUNT; i++) {
            ...
            info = sdscatprintf(info, "cluster_stats_messages_%s_received:%lld\r\n", clusterGetMessageTypeString(i), server.cluster->stats_bus_messages_received[i]);
        }
        ...
        if (epoch < 0) {
            ...
            serverLog(LL_WARNING, "configEpoch set to %llu via CLUSTER SET-CONFIG-EPOCH", (unsigned long long)myself->configEpoch);
...
void restoreCommand(client *c) {
    ...
    if (((type = rdbLoadObjectType(&payload)) == -1) || ((obj = rdbLoadObject(type, &payload, key->ptr, c->db->id, NULL)) == NULL)) {
...
migrateCachedSocket *migrateGetSocket(client *c, robj *host, robj *port, long timeout) {
    ...
    if (connBlockingConnect(conn, c->argv[1]->ptr, atoi(c->argv[2]->ptr), timeout) != C_OK) {
...
void migrateCommand(client *c) {
    ...
    /* Sanity check */
    if (getLongFromObjectOrReply(c, c->argv[5], &timeout, NULL) != C_OK || getLongFromObjectOrReply(c, c->argv[4], &dbid, NULL) != C_OK) {
        return;
    }
    if (timeout <= 0)
        timeout = 1000;
...
clusterNode *getNodeByQuery(client *c, struct redisCommand *cmd, robj **argv, int argc, int *hashslot, int *error_code) {
    ...
    int is_pubsubshard = cmd->proc == ssubscribeCommand || cmd->proc == sunsubscribeCommand || cmd->proc == spublishCommand;
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            ...
            if ((migrating_slot || importing_slot) && !is_pubsubshard && lookupKeyReadWithFlags(&server.db[0], thiskey, flags) == NULL) {
                missing_keys++;
            }
        }
        getKeysFreeResult(&result);
    ...
    if (((c->flags & CLIENT_READONLY) || is_pubsubshard) && !is_write_command && nodeIsSlave(myself) && myself->slaveof == n) {
        return myself;
    }
...
void clusterRedirectClient(client *c, clusterNode *n, int hashslot, int error_code) {
    if (error_code == CLUSTER_REDIR_CROSS_SLOT) {
        ...
        addReplyErrorSds(c, sdscatprintf(sdsempty(), "-%s %d %s:%d", (error_code == CLUSTER_REDIR_ASK) ? "ASK" : "MOVED", hashslot, getPreferredEndpoint(n), port));
    }
    else {
...
int clusterRedirectBlockedClientIfNeeded(client *c) {
    if (c->flags & CLIENT_BLOCKED && (c->btype == BLOCKED_LIST || c->btype == BLOCKED_ZSET || c->btype == BLOCKED_STREAM || c->btype == BLOCKED_MODULE)) {
        dictEntry *de;
        dictIterator *di;
        ...
        if ((de = dictNext(di)) != NULL) {
            ...
            if ((c->flags & CLIENT_READONLY) && !(c->lastcmd->flags & CMD_WRITE) && nodeIsSlave(myself) && myself->slaveof == node) {
                node = myself;
            }
